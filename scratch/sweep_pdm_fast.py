...
def main():
    configs = [
        (10.0, 10.0, False, 200.0), # frozen gates + moderate coupling
        (20.0, 20.0, False, 200.0), # frozen gates + strong coupling
        (20.0, 20.0, False, 300.0), # frozen gates + strong coupling + high load
    ]
    
    for cfg in configs:
        reg_gate_w0, gate_bus_w0, we_gates, rho_load = cfg
        print(f"\nFAST SWEEP: reg_gate_w0={reg_gate_w0}, gate_bus_w0={gate_bus_w0}, we_gates={we_gates}, rho_load={rho_load}...")
        deltas, phases = try_params(reg_gate_w0, gate_bus_w0, we_gates, rho_load)
        print("  Active bits deltas:")
        active_bits = [0, 4, 5, 6, 7, 10, 11, 13, 15]
        for b in active_bits:
            print(f"    Bit {b:2d}: delta = {deltas[b]:+.4f} | phase = {phases[b]/math.pi:.2f} * pi")
