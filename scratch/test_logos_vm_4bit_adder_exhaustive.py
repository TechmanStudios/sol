...
"""
SOL LogosVM 4-Bit Serial Adder Exhaustive Verification (Level 6: Basic Software)
================================================================================
Exhaustively validates all 512 combinations of the 4-bit serial adder under
...
class InstrumentedSequencer(MicroInstructionSequencer):
    """
    Subclass of MicroInstructionSequencer to track detailed physical invariants
    (e.g., active register mass decay) during step execution.
    """
    ...
    def record_telemetry(self):
        super().record_telemetry()
        group = self.group
        # Monitor total active register mass (host + battery) during active windows
        for reg in ['A', 'B', 'C', 'D']:
            bat_node = group.get_node(f"S_R{reg}_B")
            host_node = group.get_node(f"S_R{reg}")
            
            # If the register is active (b_state == 1)
...
def run_exhaustive_trial(x: int, y: int, cin: bool, program: list[Instruction]) -> dict:
    group = build_group()
...
def multiprocessing_worker(args):
    ...
    try:
        out = run_exhaustive_trial(x, y, cin, program)
        expected_sum = x + y + int(cin)
        expected_s = expected_sum & 0xF
        ...
        arithmetic_ok = (actual_s == expected_s) and (actual_cout == expected_cout)
        reg_ok = out["reg_ok"]
...
def main():
    print("==========================================================================")
    print("  SOL LOGOSVM 4-BIT SERIAL ADDER EXHAUSTIVE VERIFICATION SUITE (PARALLEL)")
    print("==========================================================================")
    ...
    # Use multiprocessing pool to parallelize execution safely
    num_cores = multiprocessing.cpu_count()
    processes = min(4, num_cores)
    print(f"Spawning parallel workers across {processes} processes (limited to prevent CPU overload)...")
    sys.stdout.flush()
    
    trial_num = 0
    with multiprocessing.Pool(processes=processes) as pool:
        # We use imap to yield results as they complete and print progress
        for trial_res in pool.imap(multiprocessing_worker, tasks):
            trial_num += 1
            
            if "error" in trial_res:
                print(f"Trial {trial_num}/512: X={trial_res['x']}, Y={trial_res['y']}, Cin={trial_res['cin']} | ERROR: {trial_res['error']}")
            ...
            if trial_num % 32 == 0 or not trial_res["passed"]:
                print(f"Trial {trial_num}/512: X={trial_res['x']}, Y={trial_res['y']}, Cin={trial_res['cin']} | "
    ...
    report_data = {
        ...
        "run_id": f"logos_vm_4bit_adder_exhaustive_{time.strftime('%Y%m%d_%H%M%S')}",
    ...
    json_path = report_dir / "logos_vm_4bit_adder_exhaustive_results.json"
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(report_data, f, indent=2)
        
    md_path = report_dir / "logos_vm_4bit_adder_exhaustive_report.md"
