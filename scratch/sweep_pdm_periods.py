...
from test_logos_vm_level11_pdm import Level11ManifoldGroup, Level11Sequencer, MHRALevel11ProcessingManifold, SemanticManifold, UniversalManifold, Instruction, run_level11_trial
...
def evaluate_config(periods, settle_steps, query_steps, baseline_rho=15.0):
    ...
    # Calculate phase separation (Sine vs Cosine) in degrees/radians
    sep_list = []
    for f in range(4):
...
def main():
    ...
    configs = []
    # Let's try different settle steps and query steps for some periods
    for periods in candidate_periods:
        for settle in [5, 8]:
            for query in [30, 45, 60]:
                configs.append((periods, settle, query))
                
    print(f"Starting parameter sweep over {len(configs)} configurations...")
    
    best_passed = 0
    best_cfg = None
    
    for idx, (periods, settle, query) in enumerate(configs):
        try:
            passed, seps, deltas, phases = evaluate_config(periods, settle, query)
            seps_str = ", ".join([f"{s:.2f}*pi" for s in seps])
            print(f"Config {idx:2d}: periods={periods}, settle={settle}, query={query} | Passed Bits: {passed}/16 | Seps: [{seps_str}]")
            if passed > best_passed:
                best_passed = passed
                best_cfg = (periods, settle, query, passed, seps, deltas, phases)
                if passed == 16:
                    print("!!! FOUND 100% WORKING CONFIGURATION !!!")
                    break
        except Exception as e:
            print(f"Config {idx:2d} errored: {e}")
            
    if best_cfg:
        periods, settle, query, passed, seps, deltas, phases = best_cfg
        print(f"\n========================================================")
        print(f"  BEST CONFIGURATION:")
        print(f"  Periods: {periods}")
        print(f"  Settle steps: {settle}")
        print(f"  Query steps: {query}")
        print(f"  Passed bits: {passed}/16")
        seps_str = ", ".join([f"{s:.2f}*pi" for s in seps])
        print(f"  Separations: [{seps_str}]")
        print(f"  Phases: {[p/math.pi for p in phases]}")
