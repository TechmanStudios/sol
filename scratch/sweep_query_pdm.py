...
class SweepSequencer(Level11Sequencer):
    def __init__(self, group, dt=0.08, baseline_rho=15.0, query_steps=120, settle_steps=0, load_damping=0.65, amp=8.0, match_weights=None):
...
def run_sweep_trial(val_X, val_Y, calibrated_phases, baseline_rho, query_steps, settle_steps, load_damping, amp, match_weights, c_max):
    nodes = []
    edges = []
    basins = []
    for i in range(16):
    ...
    sequencer = SweepSequencer(group, dt=0.08, baseline_rho=baseline_rho, query_steps=query_steps, settle_steps=settle_steps, load_damping=load_damping, amp=amp, match_weights=match_weights)
...
def calibrate_phases(baseline_rho, query_steps, settle_steps, load_damping, amp, match_weights, c_max):
    calibrated = [0.0] * 16
    steps = 12
    ...
    for f_idx in range(4):
        ...
        for ph in phases:
            ...
            deltas, _ = run_sweep_trial(1 << bit_sine, 0, temp_phases, baseline_rho, query_steps, settle_steps, load_damping, amp, match_weights, c_max)
        ...
        for ph in phases:
            ...
            deltas, _ = run_sweep_trial(1 << bit_cosine, 0, temp_phases, baseline_rho, query_steps, settle_steps, load_damping, amp, match_weights, c_max)
...
def calibrate_for_combo(args):
    baseline_rho, q_steps, settle_steps, load_damping, amp, match_weights, c_max = args
    calibrated = calibrate_phases(baseline_rho, q_steps, settle_steps, load_damping, amp, match_weights, c_max)
    return calibrated

def evaluate_combo(args):
    c_max, load_damping, amp, query_steps, match_weights, calibrated = args
    baseline = 15.0
    settle_steps = 0
    ...
    try:
        ...
        deltas, history = run_sweep_trial(val_X, 0, calibrated, baseline, query_steps, settle_steps, load_damping, amp, match_weights, c_max)
...
def main():
    ...
    match_weights = [10.0, 6.0, 4.0, 2.5]
    query_steps_values = [80, 100, 120, 150]
    amp_values = [6.0, 8.0]
    
    calib_tasks = []
    for q_steps in query_steps_values:
        for amp in amp_values:
            calib_tasks.append((baseline, q_steps, settle_steps, load_damping, amp, match_weights, c_max))
            
    print(f"Calibrating {len(calib_tasks)} combinations in parallel (3 workers)...", flush=True)
    t0 = time.time()
    with Pool(processes=3) as pool:
        calib_results = pool.map(calibrate_for_combo, calib_tasks)
    print(f"Calibration completed in {time.time() - t0:.2f} seconds", flush=True)
    
    tasks = []
    for idx, (baseline, q_steps, settle_steps, load_damping, amp, match_weights, c_max) in enumerate(calib_tasks):
        tasks.append((c_max, load_damping, amp, q_steps, match_weights, calib_results[idx]))
    ...
    found_any = False
    for (c_max, load_damping, amp, q_steps, _, _), (passed, res_data, min_mass) in zip(tasks, results):
        if passed:
            print(f"q_steps = {q_steps} | amp = {amp:3.1f} -> Passed = {passed} | min_mass = {min_mass:.1f}", flush=True)
            print(f"*** FOUND WORKING COMBO: q_steps={q_steps}, amp={amp} ***", flush=True)
            print(f"Deltas: {[round(d, 4) for d in res_data]}")
            found_any = True
            
    if not found_any:
        print("No combinations passed Case A completely. Dumping some results:", flush=True)
        for (c_max, load_damping, amp, q_steps, _, _), (passed, res_data, min_mass) in zip(tasks, results):
            print(f"q_steps = {q_steps} | amp = {amp:3.1f} -> min_mass = {min_mass:.1f}", flush=True)
