...
def main():
    ...
    print(f"\n========================================================")
    print(f"  SWEEP WITH toggle_nobat = True (Bits 0 to 7)")
    print(f"========================================================")
    
    for bit in range(8):
        is_cosine = (bit % 2 == 1)
        name = f"Cosine (Bit {bit})" if is_cosine else f"Sine (Bit {bit})"
        period = [10.0, 15.0, 20.0, 25.0][bit // 2]
        print(f"\nSweeping matching phase for {name} Period {period}...")
        for idx, ph in enumerate(phases):
            delta = run_trial_for_bit(bit, ph, toggle_nobat=True)
            print(f"  Phase {idx:2d}: {ph/math.pi:4.2f} * pi | delta = {delta:+.4f}")
