...
def run_pdm_trial(query_A: bool, query_B: bool, phase_A, phase_B, w0_B):
...
def main():
    num_processes = 3
    print(f"Starting parallel PDM phase sweep using {num_processes} processes...")
    
    steps = 12
    phases = [2 * math.pi * i / steps for i in range(steps)]
    
    w0_B_values = [2.0, 5.0, 10.0]
    ...
    for w0_B in w0_B_values:
        ...
        for phase_A in phases:
            for phase_B in phases:
                tasks.append((w0_B, phase_A, phase_B))
        ...
        with multiprocessing.Pool(processes=num_processes) as pool:
            for result in pool.imap_unordered(check_combination, tasks):
                if result is not None:
                    print(f"\n*** SUCCESSFUL PDM ALIGNMENT FOUND ***")
    ...
    print("\nSweep complete. No orthogonal PDM alignments found.")
    sys.exit(1)
