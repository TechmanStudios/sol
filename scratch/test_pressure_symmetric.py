...
def main():
    print("Testing 16-resonator PDM with pressure-symmetric bus modulation & disabled Jeans/semantic configs...")
    val_X = 0b1010110011110001  # Case A word: bits 0, 4, 5, 6, 7, 10, 11, 13, 15 active
    ...
    # Redefine register hosts/batteries to have semanticMass = 1.0
    ...
    # Redefine register host-battery edges to use w0 = 0.5 * (omega^2)
    ...
    # Set all register bits to baseline state
    for reg in ['X', 'Y']:
        for b in range(16):
    ...
    amp = 8.0  # Modulation amplitude
    
    for s in range(150):
        t = s * sequencer.dt
        
        # Modulate Bus with pressure-symmetric density
    ...
    for s in range(120):
        ...
        # Open and modulate ALL register gates linearly during query
        for reg in ['X', 'Y']:
            for b in range(16):
                lane = b // 8
        ...
        for b in range(16):
            ...
            # Match gates are phase-selective rectifiers!
            omega, phase_val = sequencer.get_bit_params(b)
