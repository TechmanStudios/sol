#!/usr/bin/env python3
"""
SOL Analog Conduit Multiplication with Pressure Compensation
============================================================
Dynamically compensates the Source baseline density to neutralize the
Jensen's inequality pressure-draining effect, allowing pure multiplication scaling.
"""
...
def find_compensated_rho0(A1: float, A2: float, omega1: float, omega2: float, dt: float, steps: int, target_avg_p: float, c_press: float) -> float:
    """Find the baseline density rho_0 that yields target_avg_p average pressure under oscillation."""
    low = 10.0
    high = 30.0
    for _ in range(25):
        mid = (low + high) / 2.0
...
def run_multiplication_trial(A1: float, A2: float, dt: float, steps: int, c_press: float, damping: float, phase_offset: float, active_demod: bool, rho0: float) -> float:
...
def main():
    print("======================================================================")
    print("  SOL MANIFOLD MULTIPLICATION WITH PRESSURE COMPENSATION")
    print("======================================================================")
    ...
    # Calculate target average pressure (no oscillation, rho=10.0)
    target_avg_p = c_press * math.log(1.0 + 10.0) # ~ 4.79579
    ...
    for p_idx in range(16):
        ...
        active_mass = run_multiplication_trial(2.0, 2.0, dt, steps, c_press, damping, phase, True, rho0_cal)
        ref_mass = run_multiplication_trial(2.0, 2.0, dt, steps, c_press, damping, phase, False, rho0_cal)
        diff = active_mass - ref_mass
        print(f"    Phase = {phase:5.3f} rad | Active: {active_mass:.6f} | Ref: {ref_mass:.6f} | Net: {diff:+.6f}")
        if abs(diff) > abs(best_diff):
            best_diff = diff
            best_phase = phase
            
    print(f"\n  --> Calibrated Phase Offset: {best_phase:.4f} rad\n")
    ...
    for c in cases:
        ...
        active_mass = run_multiplication_trial(c["A1"], c["A2"], dt, steps, c_press, damping, best_phase, True, rho0)
        ref_mass = run_multiplication_trial(c["A1"], c["A2"], dt, steps, c_press, damping, best_phase, False, rho0)
    ...
    if passed:
        print("\n  [STATUS] SUCCESS: Compensated analog multiplication proof verified!")
    else:
        print("\n  [STATUS] FAILED: Calibration error exceeds tolerance.")
    print("======================================================================")
