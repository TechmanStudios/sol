...
def main():
    print("Testing ultimate 16-resonator PDM architecture...")
    ...
    # Keep semanticMass = 20.0 and initial density = 300.0 (baseline_rho * 20.0)
    for n in processing.nodes:
        if n["group"] == "processing" and n["id"].startswith("S_R"):
            n["semanticMass"] = 20.0
            n["semanticMass0"] = 20.0
            n["rho"] = baseline_rho * 20.0
            
    # Tuned w0 for semanticMass = 20.0
    for e in processing.edges:
        if e["from"].startswith("S_R") and e["to"].endswith("_B"):
            ...
            w0_tuned = 10.0 * (omega ** 2) # matches test_logos_vm_level11_pdm_final.py
            e["w0"] = w0_tuned
    ...
    # Ensure isBattery = False and psi is neutralized
    for reg in ['X', 'Y']:
        for b in range(16):
    ...
    for b in range(16):
        ...
        print(f"Bit {b:2d} ({'Active' if expected else 'Flat'}): delta = {delta:+.6f} | Status = {status}")
