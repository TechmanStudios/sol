...
def main():
    print("Testing LOAD and QUERY starting from baseline...")
    val_X = 1  # Only Bit 0 is active (1), all other bits are inactive (0)
    ...
    group.engine.physics.conductance_max = 4000.0  # High conductance max to avoid clamping tuned resonator weights!
    group.engine.physics.conductance_gamma = 6.0   # Sharp rectification!
    ...
    for s in range(120):
        ...
        # Open and modulate ALL gates for active registers during query
        for reg in ['X', 'Y']:
            for b in range(16):
                lane = b // 8
