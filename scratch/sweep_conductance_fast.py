...
def main():
    ...
    c_max_values = [100.0, 200.0, 500.0, 1000.0, 4000.0]
    ...
    for c_max in c_max_values:
        for f_idx in [0, 1, 2, 3]:
            ...
            for ph in phases:
                ...
                tasks.append((c_max, (1 << bit_sine), 0, temp_phases, baseline, 150, 0, load_damping, bit_sine))
                task_keys.append((c_max, f_idx, "sine", ph))
                
            # Cosine tasks
            for ph in phases:
                temp_phases = [0.0] * 16
                temp_phases[bit_cosine] = ph
                tasks.append((c_max, (1 << bit_cosine), 0, temp_phases, baseline, 150, 0, load_damping, bit_cosine))
                task_keys.append((c_max, f_idx, "cosine", ph))
    ...
    results_map = {}
    for (c_max, f_idx, mode, ph), delta in zip(task_keys, results):
        if (c_max, f_idx) not in results_map:
            results_map[(c_max, f_idx)] = {"sine": {}, "cosine": {}}
        results_map[(c_max, f_idx)][mode][ph] = delta
        
    for c_max in c_max_values:
        print(f"=== Testing conductance_max = {c_max} ===")
        for f_idx in range(4):
            p = periods[f_idx]
            sine_data = results_map[(c_max, f_idx)]["sine"]
            best_phase_sine = max(sine_data, key=sine_data.get)
            max_delta_sine = sine_data[best_phase_sine]
            
            cosine_data = results_map[(c_max, f_idx)]["cosine"]
