...
def evaluate_trial(args):
    val_X, val_Y, temp_phases, baseline, query_steps, settle_steps, load_damping, bit_idx = args
...
def main():
    ...
    damping_values = [0.3, 0.4, 0.5, 0.6, 0.7]
    ...
    for d in damping_values:
        for f_idx in [0, 1, 2, 3]:
            ...
            for ph in phases:
                ...
                tasks.append(((1 << bit_sine), 0, temp_phases, baseline, 150, 0, d, bit_sine))
                task_keys.append((d, f_idx, "sine", ph))
                
            # Cosine tasks
            for ph in phases:
                temp_phases = [0.0] * 16
                temp_phases[bit_cosine] = ph
                tasks.append(((1 << bit_cosine), 0, temp_phases, baseline, 150, 0, d, bit_cosine))
                task_keys.append((d, f_idx, "cosine", ph))
    ...
    # Process results
    results_map = {}
    for (d, f_idx, mode, ph), delta in zip(task_keys, results):
        if (d, f_idx) not in results_map:
            results_map[(d, f_idx)] = {"sine": {}, "cosine": {}}
        results_map[(d, f_idx)][mode][ph] = delta
        
    for d in damping_values:
        print(f"=== Testing Damping = {d} ===")
        for f_idx in range(4):
            p = periods[f_idx]
            bit_sine = 2 * f_idx
            bit_cosine = 2 * f_idx + 1
            
            sine_data = results_map[(d, f_idx)]["sine"]
            best_phase_sine = max(sine_data, key=sine_data.get)
            max_delta_sine = sine_data[best_phase_sine]
            
            cosine_data = results_map[(d, f_idx)]["cosine"]
