// Phase 3.11 — 16BN19_holdLadderWashoutBetweenPasses_v1
// Goal: test whether ladder hysteresis / cross-pass carryover can be erased by a neutral washout.
//
// Protocol:
...
// - PASS1: DESC (800->0)
// - WASHOUT: relax-only ticks (no injection) between passes
// - PASS2: ASC (0->800)
...
(async () => {
  ...
  const CFG = {
    expIdBase: "sol_phase311_16bn19_holdLadderWashoutBetweenPasses_v1",
    ...
    // KEY VARIABLE: washout between passes
    washoutBetweenPassTicks: 4000, // increase to 8000 if memory still persists
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[BN19] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[BN19] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[BN19] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[BN19] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  const runAll = async (phy, pressCUsed) => {
    ...
    console.log("\n[BN19] START", runTag);
    console.log("[BN19] washoutBetweenPassTicks =", CFG.washoutBetweenPassTicks);
    ...
    try {
      for (const dirTag of CFG.dirs) {
        console.log(`\n[BN19] DIR=${dirTag} | baseline restore + modeSelect + precondition (one-time)`);
        ...
        for (let pi = 0; pi < passes.length; pi++) {
          ...
          console.log(`\n[BN19] PASS ${pass.passLabel} begins for DIR=${dirTag}`);
          ...
          for (let ci = 0; ci < pass.order.length; ci++) {
            ...
            console.log(`[BN19]  Condition ${conditionIndex}/${pass.order.length} hold=${holdTicks} reps=${CFG.repsPerCondition}`);
            ...
            for (let rep = 1; rep <= CFG.repsPerCondition; rep++) {
              ...
              if (r.aborted) throw new Error("[BN19] Aborted due to non-finite values.");
            }
          }

          // WASHOUT ONLY BETWEEN PASS1 and PASS2
          if (pi === 0 && CFG.washoutBetweenPassTicks > 0) {
            console.log(`\n[BN19] WASHOUT between passes: ${CFG.washoutBetweenPassTicks} ticks (no injections)`);
