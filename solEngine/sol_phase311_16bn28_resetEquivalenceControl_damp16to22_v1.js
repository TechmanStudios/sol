// Phase 3.11 — 16BN28_resetEquivalenceControl_damp16to22_v1
// Goal: Define reset properly by adding ASC-first control.
//
// For each damp (16..22) and each dir (up/down):
//   Seq C (ASC_ONLY_CONTROL): baseline restore + modeSelect + precondition -> ASC probe
//   Seq A (NO_RESET):         baseline restore + modeSelect + precondition -> DESC -> ASC probe
//   Seq B (RESET):            baseline restore + modeSelect + precondition -> DESC -> (PRECONDITION ONLY) -> ASC probe
//
...
(async () => {
  ...
  const CFG = {
    expIdBase: "sol_phase311_16bn28_resetEquivalenceControl_damp16to22_v1",
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[BN28] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[BN28] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[BN28] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[BN28] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  const runAll = async (phy, pressCUsed) => {
    ...
    const sequences = [
      ...
      { seqId: "B", seqLabel: "RESET_PRECONDITION_ONLY", mode: "DESC_RESET_ASC" }
    ];

    try {
      for (const dampUsed of CFG.dampList) {
        console.log(`\n[BN28] === dampUsed=${dampUsed} ===`);

        for (const dirTag of CFG.dirs) {
          console.log(`[BN28] DIR=${dirTag}`);

          for (const seq of sequences) {
            console.log(`[BN28]  SEQ ${seq.seqLabel} | baseline restore + modeSelect + precondition(initial)`);
            ...
            if (seq.mode === "ASC_ONLY") {
              console.log(`[BN28]   PASS ASC_ONLY | holds=${CFG.holdsAscProbe.join(",")} | reps=${CFG.repsPerCondition}`);
              await runASC();
              continue;
            }

            // PASS DESC
            console.log(`[BN28]   PASS DESC | holds=${CFG.holdsDesc.join(",")} | reps=${CFG.repsPerCondition}`);
            ...
            // BETWEEN
            if (seq.mode === "DESC_RESET_ASC") {
              console.log(`[BN28]   BETWEEN: Precondition ONLY`);
              await runPreconditionOnce(phy, pressCUsed, dampUsed, dirTag, baseB, ampD);
            } else {
              console.log(`[BN28]   BETWEEN: (none)`);
            }

            // PASS ASC probe
            console.log(`[BN28]   PASS ASC(probe) | holds=${CFG.holdsAscProbe.join(",")} | reps=${CFG.repsPerCondition}`);
            await runASC();
          }
        }
      }
