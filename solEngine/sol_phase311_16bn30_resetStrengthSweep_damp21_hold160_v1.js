// Phase 3.11 — 16BN30_resetStrengthSweep_damp21_hold160_v1
// Target: damp21, and specifically ASC hold160 behavior after a single strong DESC bias.
// We compare reset strength options, including a mid-strength reprime.
//
// Sequences per (dir):
//   C ASC_ONLY_CONTROL: baseline -> modeSelect -> precondition -> ASC(hold160)
//   A NO_RESET:         baseline -> modeSelect -> precondition -> DESC(hold800) -> ASC(hold160)
//   B RESET_SOFT:       baseline -> modeSelect -> precondition -> DESC(hold800) -> precondition -> ASC(hold160)
//   D RESET_HARD:       baseline -> modeSelect -> precondition -> DESC(hold800) -> (modeSelect+precondition) -> ASC(hold160)
//   E RESET_MID:        baseline -> modeSelect -> precondition -> DESC(hold800) -> (partial modeSelect + precondition) -> ASC(hold160)
//
...
(async () => {
  ...
  const CFG = {
    expIdBase: "sol_phase311_16bn30_resetStrengthSweep_damp21_hold160_v1",
    dt: 0.12,
    pressCBase: null,
    ...
    // Mode select defaults (FULL_REPRIME)
    wantId: 82,
    injectorIds: [90, 82],
    dreamBlocks: 15,
    dreamBlockSteps: 2,
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[BN30] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[BN30] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[BN30] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[BN30] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  const modeSelectOnce = async (phy, pressC, dampUsed, opts = {}) => {
    const dreamBlocks = Number.isFinite(opts.dreamBlocks) ? opts.dreamBlocks : CFG.dreamBlocks;
    const injectAmount = Number.isFinite(opts.injectAmount) ? opts.injectAmount : CFG.injectAmount;
  ...
  const runAll = async (phy, pressCUsed) => {
    ...
    const sequences = [
      { seqId: "C", seqLabel: "ASC_ONLY_CONTROL", between: "NONE" },
      { seqId: "A", seqLabel: "NO_RESET", between: "NONE" },
      { seqId: "B", seqLabel: "RESET_PRECONDITION_ONLY", between: "PRE" },
      { seqId: "D", seqLabel: "RESET_FULL_REPRIME", between: "FULL" },
      { seqId: "E", seqLabel: "RESET_PARTIAL_REPRIME", between: "MID" }
    ];

    try {
      for (const dampUsed of CFG.dampList) {
        console.log(`\n[BN30] === dampUsed=${dampUsed} ===`);
        for (const dirTag of CFG.dirs) {
          console.log(`[BN30] DIR=${dirTag}`);

          for (const seq of sequences) {
            const ctx = { _snap: null };
            await baselineRestore(phy, ctx);
            ...
            const runASC160 = async () => {
              for (let rep = 1; rep <= CFG.repsPerCondition; rep++) {
                ...
                await runOneRep(
                  ...
                  2, "ASC", 1, CFG.holdAsc, "ASC_160",
                  dirTag, rep, globalRepIndex,
                  baseB, ampD, edgeIdx, "SOLBaseline.restore",
                  summaryLines, traceLines, curveLines, runTag
                );
              }
            };
            ...
            if (seq.seqId === "C") {
              console.log(`[BN30]  SEQ C ASC_ONLY_CONTROL | ASC hold160 reps=${CFG.repsPerCondition}`);
              await runASC160();
              continue;
            }

            console.log(`[BN30]  SEQ ${seq.seqId} ${seq.seqLabel} | DESC hold800 reps=${CFG.repsPerCondition}`);
            for (let rep = 1; rep <= CFG.repsPerCondition; rep++) {
              globalRepIndex++;
              await runOneRep(
                phy, pressCUsed, dampUsed,
                seq.seqId, seq.seqLabel,
                1, "DESC", 1, CFG.holdDesc, "DESC_800",
                dirTag, rep, globalRepIndex,
                baseB, ampD, edgeIdx, "SOLBaseline.restore",
                summaryLines, traceLines, curveLines, runTag
              );
            ...
            if (seq.between === "PRE") {
              console.log(`[BN30]   BETWEEN: precondition-only`);
              await runPreconditionOnce(phy, pressCUsed, dampUsed, dirTag, baseB, ampD);
            } else if (seq.between === "FULL") {
              console.log(`[BN30]   BETWEEN: full reprime (modeSelect+precondition)`);
              await modeSelectOnce(phy, pressCUsed, dampUsed);
              await runPreconditionOnce(phy, pressCUsed, dampUsed, dirTag, baseB, ampD);
            } else if (seq.between === "MID") {
              console.log(`[BN30]   BETWEEN: partial reprime (dreamBlocks=${CFG.mid_dreamBlocks}, injectAmount=${CFG.mid_injectAmount}) + precondition`);
              await modeSelectOnce(phy, pressCUsed, dampUsed, { dreamBlocks: CFG.mid_dreamBlocks, injectAmount: CFG.mid_injectAmount });
              await runPreconditionOnce(phy, pressCUsed, dampUsed, dirTag, baseB, ampD);
            } else {
              console.log(`[BN30]   BETWEEN: none`);
            }

            console.log(`[BN30]  ASC hold160 reps=${CFG.repsPerCondition}`);
            await runASC160();
          }
        }
      }
