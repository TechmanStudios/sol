// Phase 3.11 — 16BN6 (Parity truth table, move the "clock edge" / read tick)
// Goal: Repeat BN5 parity truth table at two different markerTicks to test metastability vs settle-time.
// Runs back-to-back for markerTick = 10 and markerTick = 12.
//
// Points: multB = 0.9700 (above) and 0.9660 (below)
// Directions: BOTH (up + down)
// Modes: M0=0 primes, M1=1 prime, M2=2 primes
// Reps/cell: 120
//
// Exports per markerTick:
//   - ..._MASTER_summary.csv
//   - ..._MASTER_busTrace.csv
//   - ..._phaseParity_truthTable.csv
//
...
(async () => {
  ...
  const BASE = {
    expId: "sol_phase311_16bn6_parityTruthTable_clockEdgeShift_v1",
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[16BN6] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[16BN6] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[16BN6] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[16BN6] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  const runOneTrial = async (
    ...
    for (let tick = 0; tick < cfg.segmentTicks; tick++) {
      ...
      if (tick === cfg.markerTick) { tM_abs114 = abs114; tM_abs136 = abs136; }
      ...
      traceLines.push(csvRow([
        cfg.expId, runTag, cfg.markerTick, dirTag, cellId, modeLabel, primeCount, rep, multBUsed, tick,
    ...
    const winner_tM = (tM_abs114 === "" || tM_abs136 === "") ? 0 : (tM_abs114 > tM_abs136) ? 114 : (tM_abs136 > tM_abs114) ? 136 : 0;
    ...
    summaryLines.push(csvRow([
      cfg.expId, runTag, cfg.markerTick, dirTag, cellId, modeLabel, primeCount, rep, multBUsed,
      pressC, cfg.dampUsed, (cap.capLawHash ?? ""),
      ampB0, ampD, ratioBD,
      peakAbs114, peakAbs136, winner_peakBus,
      tM_abs114, tM_abs136, winner_tM,
    ...
    return { aborted, winner_tM, winner_peakBus, tM_abs114, tM_abs136, cap114, cap136 };
  };

  // ---------- Single markerTick run ----------
  const runForMarkerTick = async (phy, markerTick) => {
    const cfg = { ...BASE, markerTick };
    ...
    const runTag = `${cfg.expId}_mt${markerTick}_${startTag}`;
    ...
    const summaryHeader = [
      "schema","runTag","markerTick","dir","cellId","modeLabel","primeCount","rep","multBUsed",
      "pressCUsed","dampUsed","capLawHash",
      "ampB0","ampD","ratioBD",
      "peakAbs114_bus","peakAbs136_bus","winner_peakBus",
      "tM_abs114_bus","tM_abs136_bus","winner_tM",
    ...
    const traceHeader = [
      "schema","runTag","markerTick","dir","cellId","modeLabel","primeCount","rep","multBUsed","tick",
    ...
    const truthHeader = [
      "schema","runTag","markerTick","dir","multB","modeLabel","primeCount",
      "n","n114_tM","n136_tM","nTie_tM","p114_tM","p136_tM",
      "n114_peak","n136_peak","nTie_peak","p114_peak","p136_peak",
      "median_tM_abs114","median_tM_abs136",
    ...
    console.log(`\n[${cfg.expId}] START markerTick=${markerTick} @ ${startTag}`);
    ...
    for (const dirTag of dirs) {
      for (const multB of cfg.multB_points) {
        ...
        for (const primeCount of cfg.primeCounts) {
          ...
          const cellId = `${dirTag}_B${multBUsed.toFixed(4)}_${modeLabel}_mt${markerTick}`;
          ...
          const winners_tM = [];
          const winners_peak = [];
          const tM_abs114_list = [];
          const tM_abs136_list = [];
          const cap114_list = [];
          const cap136_list = [];
          ...
          for (let rep = 1; rep <= cfg.repsPerCell; rep++) {
            ...
            winners_tM.push(r.winner_tM);
            winners_peak.push(r.winner_peakBus);
            tM_abs114_list.push((r.tM_abs114 === "" ? NaN : Number(r.tM_abs114)));
            tM_abs136_list.push((r.tM_abs136 === "" ? NaN : Number(r.tM_abs136)));
          ...
          const n = winners_tM.length || 1;
          const n114_tM = winners_tM.filter(w => w === 114).length;
          const n136_tM = winners_tM.filter(w => w === 136).length;
          const nTie_tM = winners_tM.filter(w => w === 0).length;
          ...
          const p114_tM = n114_tM / n;
          const p136_tM = n136_tM / n;

          const p114_peak = n114_peak / n;
          const p136_peak = n136_peak / n;

          const med_tM_abs114 = medianNum(tM_abs114_list);
          const med_tM_abs136 = medianNum(tM_abs136_list);

          const med_cap114 = medianNum(cap114_list);
          const med_cap136 = medianNum(cap136_list);

          truthLines.push(csvRow([
            cfg.expId, runTag, markerTick, dirTag, multBUsed.toFixed(4), modeLabel, primeCount,
            n, n114_tM, n136_tM, nTie_tM, p114_tM, p136_tM,
            n114_peak, n136_peak, nTie_peak, p114_peak, p136_peak,
            med_tM_abs114, med_tM_abs136,
            med_cap114, med_cap136
          ]));

          console.log(`[${cfg.expId}] mt${markerTick} ${cellId} | p114_tM=${p114_tM.toFixed(4)} (114=${n114_tM}/${n}) | p114_peak=${p114_peak.toFixed(4)}`);
        }
      }
    }

    const endTag = isoTag(new Date());
    const baseName = `${cfg.expId}_mt${markerTick}_${startTag}_${endTag}`;
    ...
    console.log(`\n[${cfg.expId}] DONE markerTick=${markerTick} @ ${endTag}`);
  ...
  try {
    for (const mt of MARKER_TICKS) {
      await runForMarkerTick(phy, mt);
    }
  } finally {
    unfreezeLiveLoop();
  }

  console.log("✅ 16BN6 complete (both markerTick runs).");
})();
