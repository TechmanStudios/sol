// Phase 3.11 — 16BN17_holdLadderThreshold_v1
// Goal: map hold-duration threshold behavior using a hold ladder, with ASC then DESC passes in one session.
// - baseline restore ONCE per dir
// - modeSelect ONCE per dir
// - precondition ONCE per dir
// - NO restore between holds or between passes
// - repsPerCondition reduced to 10 to keep output size ~BN15/BN16
//
// Exports:
//   *_MASTER_summary.csv
...
(async () => {
  ...
  // -----------------------------
  const CFG = {
    expIdBase: "sol_phase311_16bn17_holdLadderThreshold_v1",

    dt: 0.12,
    dampUsed: 20,
    pressCBase: null,

    // Fixed operating point (same as BN15/BN16 family)
    multBFixed: 0.96625,
    segmentTicks: 401,
    ...
    // Optional settle between conditions (kept 0 to preserve continuity)
    ...
    // Precondition band (dir-dependent; same as BN15/BN16)
    preLow_multB: 0.90,
    preLow_ticks: 320,
    preHigh_multB: 1.08,
    preHigh_ticks: 320,
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[BN17] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[BN17] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[BN17] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[BN17] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  // -----------------------------
  // Runner (ASC then DESC, no restore between)
  // -----------------------------
  const runAll = async (phy, pressCUsed) => {
    ...
    const passes = [
      { passId: 1, passLabel: "ASC",  order: asc,  orderLabel: "ASC_"  + asc.join("_")  },
      { passId: 2, passLabel: "DESC", order: desc, orderLabel: "DESC_" + desc.join("_") }
    ];

    console.log("\n[BN17] START", runTag);
    console.log("[BN17] Holds ASC =", asc.join(" -> "));
    console.log("[BN17] Holds DESC=", desc.join(" -> "));
    console.log("[BN17] Baseline restore ONCE per dir; no restore between holds/passes.");
    ...
    try {
      for (const dirTag of CFG.dirs) {
        console.log(`\n[BN17] DIR=${dirTag} | baseline restore + modeSelect + precondition (one-time)`);
        const baselineModeUsed = await (async () => {
          if (window.SOLBaseline?.restore) { await window.SOLBaseline.restore(); return "SOLBaseline.restore"; }
          if (!ctx._snap) ctx._snap = makeInternalSnapshot(phy);
          restoreInternalSnapshot(phy, ctx._snap);
          return "internal_snapshot_restored";
        ...
        for (const pass of passes) {
          console.log(`\n[BN17] PASS ${pass.passLabel} begins for DIR=${dirTag}`);
          ...
          for (let ci = 0; ci < pass.order.length; ci++) {
            ...
            console.log(`[BN17]  Condition ${conditionIndex}/${pass.order.length} hold=${holdTicks} reps=${CFG.repsPerCondition}`);
            ...
            for (let rep = 1; rep <= CFG.repsPerCondition; rep++) {
              ...
              if (r.aborted) {
                console.warn(`[BN17] ABORT: dir=${dirTag} pass=${pass.passLabel} hold=${holdTicks} rep=${rep} (non-finite).`);
                throw new Error("[BN17] Aborted due to non-finite values.");
              }

              if (rep === 1 || rep === 2 || rep === 3 || rep === 5 || rep === 10) {
                console.log(
                  `[BN17]   dir=${dirTag} pass=${pass.passLabel} hold=${holdTicks} rep=${rep} |`
                  + ` lateUp80=${r.lateUp114_80} peak=${r.winner_peakBus}`
                );
              }
            }
          }
