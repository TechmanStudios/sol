/* Phase 3.11 — 16AW: Dense Boundary Microscope (static phase diagram, baseline restore every run)
   Purpose:
     Map the turbulent band precisely for baselineFp slices like eaf96241.
     Measures BOTH:
       - winner_peakBus (eventual capture)
       - winner_t8 (early capture)
     + laneEdge mode + laneEdge_count (order parameter)

   Fixed:
     - dampUsed = 20
     - Order B2: 136@t0, 114@t1
     - Bus arbitration by abs flux on {->89, ->79}

   Sweep (default):
     multBUsedList = [0.93,0.94,0.945,0.95,0.955,0.96,0.965,0.97,0.975,0.98]
     repsPerCond   = 120
     shuffle       = true
   ...
   Run:
     await solPhase311_16aw_boundaryMicroscopeDense_v1.run()

   Stop:
     solPhase311_16aw_boundaryMicroscopeDense_v1.stop()
*/
(() => {
  "use strict";

  const T = {
    version: "sol_phase311_16aw_boundaryMicroscopeDense_v1",
    _stop: false,
    ...
    cfg: {
      ...
      label: "B2_d20_denseBoundaryMicroscope"
    },
    ...
    async _waitForPhysics(timeoutMs = 15000, pollMs = 50){
      ...
      throw new Error("[16aw] timed out waiting for physics.");
    },

    _freezeLiveLoop(){
      const app = this._getApp();
      if (!app?.config) throw new Error("[16aw] App not ready.");
    ...
    _inject(phy, id, amt){
      ...
      if (!n) throw new Error(`[16aw] node not found: ${id}`);
    ...
    _baselineFingerprint(phy){
      ...
      for (const id of ids){
        ...
        const vals = [n.x,n.y,n.vx,n.vy,n.fx,n.fy].map(v => (Number.isFinite(v) ? v.toFixed(6) : "NA"));
        parts.push(`${id}:${vals.join("|")}`);
      }
      const gb = ("globalBias" in phy) ? String(phy.globalBias) : "NA";
      return this._fnv1a32(`gb=${gb}::${parts.join("::")}`);
    },
    ...
    async run(userCfg = {}){
      ...
      console.log(`[16aw] START ${this.version} @ ${startTag} | pressC=${pressCUsed} | damp=${g.dampUsed} | runs=${plan.length}`);

      await this._baselineRestore(phy);
      const baselineFp = this._baselineFingerprint(phy);
      console.log(`[16aw] baselineFp=${baselineFp}`);
      ...
      for (let runIndex = 0; runIndex < plan.length; runIndex++){
        ...
        const runId = `${this._iso(new Date())}_fp${baselineFp}_r${String(runIndex).padStart(6,"0")}_mB${multBUsed.toFixed(3)}_rep${rep}`;
        ...
        const runQuality = (lane.count >= g.laneEdgeCountGoodMin && !aborted) ? "good" : "unstable";
        ...
        if ((runIndex + 1) % 100 === 0) console.log(`[16aw] progress ${(runIndex + 1)}/${plan.length}`);
      }
      ...
      console.log(`[16aw] DONE @ ${endTag}\n- ${baseName}_MASTER_summary.csv\n- ${baseName}_MASTER_busTrace.csv`);
      return { baseName, baselineFp, runsPlanned: plan.length, stopped: this._stop };
    }
  };

  window.solPhase311_16aw_boundaryMicroscopeDense_v1 = T;
  console.log(`✅ solPhase311_16aw_boundaryMicroscopeDense_v1 installed. Run: await solPhase311_16aw_boundaryMicroscopeDense_v1.run()`);
})();
