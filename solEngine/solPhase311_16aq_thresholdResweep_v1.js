/* Phase 3.11.16AQ — Threshold Resweep under Baseline v1.5 (B2 damp20)
   Goal:
     Re-map the arbitration flip boundary under the stricter baseline (v1.5),
     since v1.4/v1.5 can shift the operating point by removing hidden memory.
   ...
   Sweep:
     - multBUsed ∈ [1.44, 1.60, 1.80, 2.00, 2.20]
     - repsPerCond = 40
     - shuffle = true
   ...
   Run:
     await solPhase311_16aq_thresholdResweep_v1.run()

   Stop:
     solPhase311_16aq_thresholdResweep_v1.stop()

   Notes:
     - Uses await SOLBaseline.restore() if available; else internal snapshot fallback.
...
(() => {
  ...
  const R = {
    version: "sol_phase311_16aq_thresholdResweep_v1",
    _stop: false,
    ...
    cfg: {
      ...
      // Sweep
      multBUsedList: [1.44, 1.60, 1.80, 2.00, 2.20],
      repsPerCond: 40,
      shuffle: true,

      // Label
      label: "B2_d20_thresholdResweep_v15"
    },
    ...
    async _waitForPhysics(timeoutMs = 15000, pollMs = 50) {
      ...
      throw new Error("[16aq] timed out waiting for physics.");
    },

    _freezeLiveLoop() {
      const app = this._getApp();
      if (!app?.config) throw new Error("[16aq] App not ready.");
    ...
    _inject(phy, id, amt) {
      ...
      if (!n) throw new Error(`[16aq] node not found: ${id}`);
      const a = Math.max(0, Number(amt) || 0);
      n.rho += a;

      // Keep reinforcement ON (baseline v1.5 should now restore the relevant state)
      try {
    ...
    _restore(phy, snap) {
      ...
      try {
        if ("_t" in phy && snap.nodes?.length) {
          const anyNode = snap.nodes[0]?.[1];
          if (anyNode && typeof anyNode._t === "number" && Number.isFinite(anyNode._t)) phy._t = anyNode._t;
        }
      } catch (e) {}
    },
    ...
    async run(userCfg = {}) {
      ...
      console.log(`[16aq] START ${this.version} @ ${startTag} | pressCBase=${pressCBase} | runs=${plan.length}`);
      ...
      for (const item of plan) {
        ...
        const runId =
          `${this._iso(new Date())}_r${String(runIndex).padStart(6, "0")}_mB${Number(multBUsed).toFixed(2)}_rep${repIndex}`;

        const baselineMode = await this._baselineRestore(phy);
        const cap = await this._recomputeDerived(g.dt);

        await this._modeSelect(phy, pressCBase, g.dampUsed);
        for (let s = 0; s < g.settleTicks; s++) phy.step(g.dt, pressCBase, g.dampUsed);
        ...
        for (let tick = 0; tick < g.totalTicks; tick++) {
          ...
          phy.step(g.dt, pressCBase, g.dampUsed);
          ...
          if (arbiter_tick == null && busSet.has(max1Pair)) {
            ...
            abs114_89_atArb = Math.abs(f114_89);
            abs136_89_atArb = Math.abs(f136_89);
            deltaAbs_atArb = (Math.abs(f114_89) - Math.abs(f136_89));
          ...
          // First max1-on-bus for each owner
          ...
          traceLines.push(this._csvRow([
            ...
            pressCBase, g.dampUsed, multBUsed, ampB0, ampD,
        ...
        if ((runIndex + 1) % 25 === 0) console.log(`[16aq] progress ${(runIndex + 1)}/${plan.length}`);
        runIndex += 1;
      }
      ...
      console.log(`[16aq] DONE @ ${endTag} | runs=${runIndex}\nFiles:\n- ${baseName}_MASTER_summary.csv\n- ${baseName}_MASTER_busTrace.csv`);
      return { baseName, runs: runIndex, stopped: this._stop };
    }
  };

  window.solPhase311_16aq_thresholdResweep_v1 = R;
  console.log(`✅ solPhase311_16aq_thresholdResweep_v1 installed (${R.version}). Run: await solPhase311_16aq_thresholdResweep_v1.run()`);
})();
