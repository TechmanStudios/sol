// Phase 3.11 — 16BN18_holdLadderCounterbalance_v1
// Goal: counterbalance BN17 by running DESC first, then ASC (within each dir) to separate
// ladder-hysteresis from pass-position drift.
//
// Protocol:
...
// - holds = [0,40,81,160,400,800], repsPerCondition=10
//
// Exports:
//   *_MASTER_summary.csv
...
(async () => {
  ...
  const CFG = {
    expIdBase: "sol_phase311_16bn18_holdLadderCounterbalance_v1",
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[BN18] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[BN18] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[BN18] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[BN18] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  const runAll = async (phy, pressCUsed) => {
    ...
    // Counterbalance: DESC first, then ASC
    const passes = [
    ...
    console.log("\n[BN18] START", runTag);
    console.log("[BN18] PASS1=DESC, PASS2=ASC");
    console.log("[BN18] Baseline restore ONCE per dir; no restore between holds/passes.");
    ...
    try {
      for (const dirTag of CFG.dirs) {
        console.log(`\n[BN18] DIR=${dirTag} | baseline restore + modeSelect + precondition (one-time)`);
        ...
        for (const pass of passes) {
          console.log(`\n[BN18] PASS ${pass.passLabel} begins for DIR=${dirTag}`);
          ...
          for (let ci = 0; ci < pass.order.length; ci++) {
            ...
            console.log(`[BN18]  Condition ${conditionIndex}/${pass.order.length} hold=${holdTicks} reps=${CFG.repsPerCondition}`);
            ...
            for (let rep = 1; rep <= CFG.repsPerCondition; rep++) {
              ...
              if (r.aborted) throw new Error("[BN18] Aborted due to non-finite values.");
            }
          }
        }
      }
