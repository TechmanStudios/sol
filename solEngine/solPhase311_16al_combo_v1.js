/* Phase 3.11.16AL — Combo: Threshold Refinement + Order-Dependence Check
   Runs two sections back-to-back and downloads 4 CSVs at the end.

   SECTION T1 (threshold refinement):
     - Order: 114@t0, 136@t1 (114-first)
     - dampUsed: [12, 20]
     - multBUsed: [1.20, 1.25, 1.30, 1.35, 1.40, 1.44]
     - repsPerCond: 40
     - nudgeOnlyIf136First: false

   SECTION T2 (order dependence):
     - Order: 136@t0, 114@t1 (classic order, but strong 114)
     - dampUsed: [12, 20]
     - multBUsed: [1.44, 1.70, 2.00]
     - repsPerCond: 30
     - nudgeOnlyIf136First: false

   Outputs (downloads AFTER both sections finish):
     - ..._T1_threshold_MASTER_summary.csv
     - ..._T1_threshold_MASTER_busTrace.csv
     - ..._T2_orderDepend_MASTER_summary.csv
     - ..._T2_orderDepend_MASTER_busTrace.csv

   Run:
     await solPhase311_16al_combo_v1.run()

   Stop:
     solPhase311_16al_combo_v1.stop()
...
(() => {
  ...
  const M = {
    version: "sol_phase311_16al_comboThresholdRefine_OrderDepend_v1",
    _stop: false,
    ...
    cfg: {
      ...
      // Marker moment (for lane classification)
      markerTickToCheck: 8,
      ...
      sectionT1: {
        tag: "T1_threshold",
        label: "114first_threshold_refine",
        dampUsedList: [12, 20],
        multBUsedList: [1.20, 1.25, 1.30, 1.35, 1.40, 1.44],
        repsPerCond: 40,
        shuffle: true,
        injectTick114: 0,
        injectTick136: 1,
      ...
      sectionT2: {
        tag: "T2_orderDepend",
        label: "classicOrder_strong114",
        dampUsedList: [12, 20],
        multBUsedList: [1.44, 1.70, 2.00],
        repsPerCond: 30,
        shuffle: true,
        injectTick114: 1,
        injectTick136: 0,
    ...
    async _waitForPhysics(timeoutMs = 15000, pollMs = 50) {
      ...
      throw new Error("[16al] timed out waiting for physics.");
    },

    _freezeLiveLoop() {
      const app = this._getApp();
      if (!app?.config) throw new Error("[16al] App not ready.");
    ...
    _inject(phy, id, amt) {
      ...
      if (!n) throw new Error(`[16al] node not found: ${id}`);
    ...
    async _runSection(phy, edgeIndex, pressCBase, sec) {
      ...
      // Build plan: damp x multB x reps
      const plan = [];
      for (const dampUsed of sec.dampUsedList) {
        for (const multBUsed of sec.multBUsedList) {
          for (let r = 1; r <= sec.repsPerCond; r++) plan.push({ dampUsed, multBUsed, rep: r });
        }
      }
      ...
      console.log(
        `[16al:${sec.tag}] plan=${plan.length} | damps=[${sec.dampUsedList.join(",")}] | multB=[${sec.multBUsedList.join(",")}] reps=${sec.repsPerCond}` +
        ` | inject(114@${sec.injectTick114},136@${sec.injectTick136}) | nudgeOnlyIf136First=${sec.nudgeOnlyIf136First}`
      );
      ...
      for (const item of plan) {
        ...
        for (let tick = 0; tick < g.totalTicks; tick++) {
          ...
          if (g.abortOnNonFinite) {
            ...
            if (!ok) {
              ...
              console.warn(`[16al:${sec.tag}] ABORT runIndex=${runIndex} damp=${dampUsed} mB=${multBUsed} rep=${repIndex} tick=${tick}: non-finite sample`);
              break;
            }
          }
        ...
        summaryLines.push(this._csvRow([
          ...
          g.nudgeMult, (sec.nudgeOnlyIf136First ? 1 : 0),
        ...
        // Highlight any 114-first events immediately
        if (arbiterOwner114 || precedenceStarts114) {
          console.warn(
            `[16al:${sec.tag}] 114-event runIndex=${runIndex} damp=${Number(dampUsed).toFixed(2)} mB=${Number(multBUsed).toFixed(3)} rep=${repIndex}` +
            ` | inject(114@${sec.injectTick114},136@${sec.injectTick136}) arb=${arbiter_owner}@${arbiter_tick} prec=${precedence}` +
            ` | tick8=${max1_at_markerTick} ampB0=${ampB0.toFixed(2)} ampD=${ampD.toFixed(2)}`
          );
        } else if ((runIndex + 1) % 25 === 0) {
          console.log(`[16al:${sec.tag}] progress ${(runIndex + 1)}/${plan.length}`);
        }
    ...
    async run(userCfg = {}) {
      ...
      console.log(`[16al] START ${this.version} @ ${startTag} | pressCBase=${pressCBase}`);

      const T1 = await this._runSection(phy, edgeIndex, pressCBase, this.cfg.sectionT1);
      if (this._stop) console.warn("[16al] stopped after T1.");

      const T2 = this._stop ? { summaryLines: [""], traceLines: [""], runs: 0 }
        : await this._runSection(phy, edgeIndex, pressCBase, this.cfg.sectionT2);
      ...
      this._download(`${baseName}_T1_threshold_MASTER_summary.csv`, T1.summaryLines.join(""));
      this._download(`${baseName}_T1_threshold_MASTER_busTrace.csv`, T1.traceLines.join(""));

      this._download(`${baseName}_T2_orderDepend_MASTER_summary.csv`, T2.summaryLines.join(""));
      this._download(`${baseName}_T2_orderDepend_MASTER_busTrace.csv`, T2.traceLines.join(""));
      ...
      console.log(
        ...
        `- T2 runs: ${T2.runs}\n` +
        `Files:\n` +
        `- ${baseName}_T1_threshold_MASTER_summary.csv\n` +
        `- ${baseName}_T1_threshold_MASTER_busTrace.csv\n` +
        `- ${baseName}_T2_orderDepend_MASTER_summary.csv\n` +
        `- ${baseName}_T2_orderDepend_MASTER_busTrace.csv`
      );

      return { baseName, pressCUsed: pressCBase, runsT1: T1.runs, runsT2: T2.runs, stopped: this._stop };
    }
  };

  window.solPhase311_16al_combo_v1 = M;
  console.log(`✅ solPhase311_16al_combo_v1 installed (${M.version}). Run: await solPhase311_16al_combo_v1.run()`);
})();
