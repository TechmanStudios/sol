// Phase 3.11 — 16BN27_resetReplication_damp16to22_v1
// Goal: High-confidence test of between-pass carryover vs PRECONDITION reset, in the regime where leakage exists.
//
// Damps: 16..22
// For each damp, for each dir:
//   Sequence A (NO_RESET): baseline restore + modeSelect + precondition (initial) -> DESC -> ASC
//   Sequence B (RESET):    baseline restore + modeSelect + precondition (initial) -> DESC -> (PRECONDITION ONLY) -> ASC
//
...
(async () => {
  ...
  const CFG = {
    expIdBase: "sol_phase311_16bn27_resetReplication_damp16to22_v1",

    dt: 0.12,
    pressCBase: null,

    // Sweep damp = 16..22
    dampList: Array.from({ length: 7 }, (_, i) => 16 + i),
    ...
    // busTrace sampling (safe filesize, still good fidelity)
    traceStride: 4,
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[BN27] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[BN27] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[BN27] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[BN27] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  const runSequence = async (
    phy, pressCUsed, dampUsed, dirTag,
    seqId, seqLabel, doBetweenReset,
    baseB, ampD, edgeIdx,
    summaryLines, traceLines, curveLines, runTag
  ) => {
    ...
    const descPass = { passId: 1, passLabel: "DESC", order: CFG.holdsDesc, orderLabel: "DESC_" + CFG.holdsDesc.join("_") };
    const ascPass  = { passId: 2, passLabel: "ASC",  order: CFG.holdsAscProbe, orderLabel: "ASC_" + CFG.holdsAscProbe.join("_") };

    return { baselineModeUsed, descPass, ascPass };
  };
  ...
  const runAll = async (phy, pressCUsed) => {
    ...
    console.log("\n[BN27] START", runTag);
    console.log("[BN27] damp sweep:", CFG.dampList.join(","));
    console.log("[BN27] repsPerCondition:", CFG.repsPerCondition);
    console.log("[BN27] trace sampling: stride=", CFG.traceStride, "repsPerCond=", CFG.traceRepsPerCondition);

    let globalRepIndex = 0;

    const sequences = [
      { seqId: "A", seqLabel: "NO_RESET", doBetweenReset: false },
      { seqId: "B", seqLabel: "RESET_PRECONDITION_ONLY", doBetweenReset: true }
    ];

    try {
      for (const dampUsed of CFG.dampList) {
        console.log(`\n[BN27] === dampUsed=${dampUsed} ===`);

        for (const dirTag of CFG.dirs) {
          console.log(`[BN27] DIR=${dirTag}`);

          for (const seq of sequences) {
            console.log(`[BN27]  SEQ ${seq.seqLabel} | baseline restore + modeSelect + precondition(initial)`);

            // fresh baseline for each sequence (internal comparison control)
            const ctx = { _snap: null };
            const baselineModeUsed = await baselineRestore(phy, ctx);
            await modeSelectOnce(phy, pressCUsed, dampUsed);
            await runPreconditionOnce(phy, pressCUsed, dampUsed, dirTag, baseB, ampD);

            // PASS 1: DESC full ladder
            console.log(`[BN27]   PASS DESC | holds=${CFG.holdsDesc.join(",")} | reps=${CFG.repsPerCondition}`);
            ...
            // BETWEEN PASSES
            if (seq.doBetweenReset) {
              console.log(`[BN27]   BETWEEN: Precondition ONLY (reset)`);
              await runPreconditionOnce(phy, pressCUsed, dampUsed, dirTag, baseB, ampD);
            } else {
              console.log(`[BN27]   BETWEEN: (none)`);
            }

            // PASS 2: ASC probe ladder
            console.log(`[BN27]   PASS ASC(probe) | holds=${CFG.holdsAscProbe.join(",")} | reps=${CFG.repsPerCondition}`);
