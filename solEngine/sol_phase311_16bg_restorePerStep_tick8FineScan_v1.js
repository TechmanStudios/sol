// Phase 3.11 — 16BG (restore-per-step) tick8 fine hysteresis scan
// Purpose: resolve tick8 x50 and hysteresis width near the 16BF crossing at multB≈0.990.
...
(async () => {
  ...
  const CFG = {
    expId: "sol_phase311_16bg_restorePerStep_tick8FineScan_v1",

    // Fine scan around 0.990 in 0.0005 steps (13 points)
    upList:   [0.9870,0.9875,0.9880,0.9885,0.9890,0.9895,0.9900,0.9905,0.9910,0.9915,0.9920,0.9925,0.9930],
    downList: [0.9930,0.9925,0.9920,0.9915,0.9910,0.9905,0.9900,0.9895,0.9890,0.9885,0.9880,0.9875,0.9870],
    ...
    // Directional precondition ONCE per step (same as 16BF)
    preLow_multB: 0.90,
    preLow_ticks: 320,
    preHigh_multB: 1.08,
    preHigh_ticks: 320,

    // Mode select ONCE per step (same as 16BF)
    wantId: 82,
    injectorIds: [90, 82],
    dreamBlocks: 15,
    dreamBlockSteps: 2,
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[16BG] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[16BG] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[16BG] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[16BG] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  window.solPhase311_16bg_restorePerStep_tick8FineScan_v1 = Runner;
  console.log("✅ Installed: solPhase311_16bg_restorePerStep_tick8FineScan_v1");
  await Runner.run();
