/* Phase 3.11.16AO — History-Sensitivity Probe (Baseline Leak Test)
   Purpose:
     Test whether the apparent phase boundary shifts due to hidden state not reset by baseline.
   ...
   Each rep:
     - restores baseline (SOLBaseline.restore() if available; else internal snapshot)
     - mode-select priming (Dream blocks)
     - runs tick loop and logs summary + busTrace
   ...
   Run:
     await solPhase311_16ao_historySensitivityProbe_v1.run()

   Stop:
     solPhase311_16ao_historySensitivityProbe_v1.stop()
...
(() => {
  ...
  const H = {
    version: "sol_phase311_16ao_historySensitivityProbe_v1",
    _stop: false,
    ...
    cfg: {
      ...
      // Blocks
      blocks: [
        { tag: "A_pre195",   label: "B2_d20_mB1.195_pre",  multBUsed: 1.195, reps: 60 },
        { tag: "S_stress144",label: "B2_d20_mB1.440_stress",multBUsed: 1.440, reps: 30 },
        { tag: "B_post195",  label: "B2_d20_mB1.195_post", multBUsed: 1.195, reps: 60 }
      ],
    ...
    async _waitForPhysics(timeoutMs = 15000, pollMs = 50) {
      ...
      throw new Error("[16ao] timed out waiting for physics.");
    },

    _freezeLiveLoop() {
      const app = this._getApp();
      if (!app?.config) throw new Error("[16ao] App not ready.");
    ...
    _inject(phy, id, amt) {
      ...
      if (!n) throw new Error(`[16ao] node not found: ${id}`);
    ...
    async _runBlock(phy, edgeIndex, pressCUsed, block, globalRunBaseIndex) {
      ...
      console.log(`[16ao:${block.tag}] reps=${block.reps} mB=${block.multBUsed} ampB0=${ampB0.toFixed(2)} ampD=${ampD.toFixed(2)} (damp=${g.dampUsed})`);
      ...
      for (const item of plan) {
        ...
        if ((blockRunIndex + 1) % 20 === 0) {
          console.log(`[16ao:${block.tag}] progress ${blockRunIndex + 1}/${block.reps}`);
        }
    ...
    async run(userCfg = {}) {
      ...
      console.log(`[16ao] START ${this.version} @ ${startTag} | pressC=${pressCUsed} | damp=${g.dampUsed}`);
  ...
  window.solPhase311_16ao_historySensitivityProbe_v1 = H;
  console.log(`✅ solPhase311_16ao_historySensitivityProbe_v1 installed (${H.version}). Run: await solPhase311_16ao_historySensitivityProbe_v1.run()`);
})();
