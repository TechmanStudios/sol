// Phase 3.11 — 16BL (bit-forced) HOLD-mode threshold scan
// Goal: test whether the "latched phase bit" shifts the tick8 threshold curve / x50.
//
// Protocol (per step):
//   - baseline restore (SOLBaseline.restore)
//   - modeSelect once
//   - directional precondition once
//   - BL0: HOLD only (no priming)  => default phase
//   - BL1: ONE toggle-stride prime rep, then HOLD => flipped phase
//   - Then run repsPerStep trials in HOLD mode (betweenRepTicks_hold=81)
//
// Scan band matches 16BK:
//   multB 1.0100 → 1.0160 step 0.0005, up + down.
//
// Exports:
//   - MASTER_summary.csv
//   - MASTER_busTrace.csv
...
(async () => {
  ...
  const CFG = {
    expId: "sol_phase311_16bl_bitForced_holdMode_tick8ThresholdScan_v1",
    ...
    repsPerStep: 80,          // per multB step per variant
    dt: 0.12,
    pressCBase: null,
    dampUsed: 20,
    markerTick: 8,
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[16BL] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[16BL] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[16BL] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[16BL] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  const buildRatesByDirMultB = (rows) => {
    const m = new Map();
    for (const r of rows) {
      const key = `${r.variant}|${r.dir}|${r.multB}`;
      if (!m.has(key)) m.set(key, {
        variant: r.variant, dir: r.dir, multB: r.multB, n: 0,
    ...
    out.sort((x, y) => {
      if (x.variant !== y.variant) return x.variant < y.variant ? -1 : 1;
      if (x.dir !== y.dir) return x.dir < y.dir ? -1 : 1;
      return x.multB - y.multB;
    });
    return out;
  };
  ...
  const buildThresholdEstimates = (rates) => {
    const variants = [...new Set(rates.map(r => r.variant))];
    ...
    for (const v of variants) {
      for (const met of metrics) {
        ...
        for (const d of dirs) {
          const pts = rates
            .filter(r => r.variant === v && r.dir === d)
            .sort((a,b)=>a.multB-b.multB)
            .map(r => ({ x: r.multB, p: r[met.key] }));
          ...
          rows.push({
            ...
            dir: d,
            x50: est.x50,
            bracket_lo: est.bracket[0],
            bracket_hi: est.bracket[1],
            p_lo: est.pBracket[0],
            p_hi: est.pBracket[1]
          });
        }
        if (estByDir.up?.x50 !== "" && estByDir.down?.x50 !== "") {
          rows.push({ variant: v, metric: met.name, dir: "hysteresis", x50: (estByDir.up.x50 - estByDir.down.x50), bracket_lo: "", bracket_hi: "", p_lo: "", p_hi: "" });
        }
      }
    }
    return rows;
  ...
  const buildTurbulenceProfile = (rates) => {
    ...
    for (const v of variants) {
      for (const d of dirs) {
        const pts = rates.filter(r => r.variant === v && r.dir === d).sort((a,b)=>a.multB-b.multB);
        const n = pts.length;
        const dp = (arr, i, key) => {
          if (n < 2) return "";
  ...
  const Runner = {
    ...
    async run(userCfg = {}) {
      ...
      const summaryHeader = [
        "schema","runTag","variant","dir","stepIndex","stepRep","multBUsed",
      ...
      const traceHeader = [
        "schema","runTag","variant","dir","stepIndex","stepRep","multBUsed","tick",
      ...
      const runOneTrial = async (variant, dirTag, stepIndex, rep, multBUsed, baselineModeUsed, betweenRepTicks, record=true) => {
        ...
        for (let tick = 0; tick < cfg.segmentTicks; tick++) {
          ...
          if (record) {
            traceLines.push(csvRow([
              cfg.expId, runTag, variant, dirTag, stepIndex, rep, multBUsed, tick,
        ...
        if (record) {
          summaryLines.push(csvRow([
            cfg.expId, runTag, variant, dirTag, stepIndex, rep, multBUsed,
      ...
      const primeLatch = async (variant, dirTag, multBUsed, baselineModeUsed) => {
        // BL0: no priming (holdOnly)
        // BL1: do one toggle stride trial (not recorded), then relax by toggle ticks
        if (variant !== "BL1") return;
        await runOneTrial(variant, `${dirTag}_PRIME`, -999, 0, multBUsed, baselineModeUsed, cfg.betweenRepTicks_toggle, false);
        await relax(cfg.betweenRepTicks_toggle);
      };

      const runStep = async (variant, dirTag, stepIndex, multBUsed) => {
        ...
        await primeLatch(variant, dirTag, multBUsed, baselineModeUsed);

        for (let rep = 1; rep <= cfg.repsPerStep; rep++) {
          if (this.stopFlag) break;
          const r = await runOneTrial(variant, dirTag, stepIndex, rep, multBUsed, baselineModeUsed, cfg.betweenRepTicks_hold, true);
          if (r?.aborted) break;
          await relax(cfg.betweenRepTicks_hold);
        }
      };

      const runPath = async (variant, dirTag, list) => {
        for (let s = 0; s < list.length; s++) {
          if (this.stopFlag) break;
          await runStep(variant, dirTag, s, list[s]);
        }
      };

      console.log(`\n[${cfg.expId}] START @ ${startTag}`);
      console.log(`[${cfg.expId}] HOLD betweenRepTicks=${cfg.betweenRepTicks_hold} (stride even). togglePrime=${cfg.betweenRepTicks_toggle} (stride odd).`);
      console.log(`[${cfg.expId}] band: ${cfg.upList[0]} → ${cfg.upList[cfg.upList.length-1]} step 0.0005 | repsPerStep=${cfg.repsPerStep}`);
      console.log(`[${cfg.expId}] variants: ${cfg.variants.join(", ")}`);

      for (const variant of cfg.variants) {
        if (this.stopFlag) break;
        console.log(`\n[${cfg.expId}] === Variant ${variant} ===`);
        console.log(`[${cfg.expId}] ${variant} UP...`);
        await runPath(variant, "up", cfg.upList);
        console.log(`[${cfg.expId}] ${variant} DOWN...`);
        await runPath(variant, "down", cfg.downList);
      }
      ...
      const ratesHeader = [
        "variant","dir","multB","n","p114_peak","p114_t8","cap114_rate","cap136_rate",
      ...
      const thrHeader = ["variant","metric","dir","x50","bracket_lo","bracket_hi","p_lo","p_hi"];
      const thrCsv = [csvRow(thrHeader)];
      for (const r of thrRows) thrCsv.push(csvRow([r.variant, r.metric, r.dir, r.x50, r.bracket_lo, r.bracket_hi, r.p_lo, r.p_hi]));
  ...
  window.solPhase311_16bl_bitForced_holdMode_tick8ThresholdScan_v1 = Runner;
  console.log("✅ Installed: solPhase311_16bl_bitForced_holdMode_tick8ThresholdScan_v1");
  await Runner.run();
