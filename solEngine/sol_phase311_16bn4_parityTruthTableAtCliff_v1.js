// Phase 3.11 — 16BN4 (Parity truth table right at the cliff)
// Goal: At fixed multB points near the inversion boundary, test parity control:
//   M0: no prime (0 toggles)
//   M1: one toggle prime (1 toggle)
//   M2: two toggle primes (2 toggles)
// Under HOLD sampling, does parity still select phase near the boundary?
//
// Points: multB = 0.9685 and 0.9680
// Directions: BOTH (up + down) to check any residual hysteresis.
// Reps per cell: 120 (so each p is /120)
//
// Per cell protocol:
...
//   4) PRIME: run N primes (N=0,1,2), each is one segment + relax with betweenRepTicks=80
//   5) HOLD: run repsPerCell trials, each is one segment + relax with betweenRepTicks=81
//
// Exports:
//   - MASTER_summary.csv
//   - MASTER_busTrace.csv
...
(async () => {
  ...
  const CFG = {
    expId: "sol_phase311_16bn4_parityTruthTableAtCliff_v1",

    // Core sampling
    segmentTicks: 121,
    betweenRepTicks_hold: 81,   // HOLD
    betweenRepTicks_toggle: 80, // TOGGLE (prime)
    ...
    multB_points: [0.9685, 0.9680],

    // Cells (prime parity)
    primeCounts: [0, 1, 2], // M0, M1, M2

    repsPerCell: 120,
    dirMode: "both", // "up" | "down" | "both"

    // Precondition (same as your BN-series)
    preLow_multB: 0.90,
    preLow_ticks: 320,
    preHigh_multB: 1.08,
    preHigh_ticks: 320,

    // Mode select ONCE per cell
    wantId: 82,
    injectorIds: [90, 82],
    dreamBlocks: 15,
    dreamBlockSteps: 2,
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[16BN4] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[16BN4] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[16BN4] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[16BN4] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  // ----------------------------
  // PRIME (toggle-stride rep, not recorded). Run N times.
  // ----------------------------
  ...
  // ----------------------------
  // One recorded trial (HOLD) — pushes to summary + trace
  // ----------------------------
  const runOneTrial = async (
    phy, cfg, pressC, dirTag, cellId, rep, multBUsed, baseB, ampD, edgeIdx,
    baselineModeUsed, modeLabel, primeCount, summaryLines, traceLines, runTag
  ) => {
    ...
    return { aborted, winner_t8, winner_peakBus, t8_abs114, t8_abs136 };
  };
  ...
  const Runner = {
    async run(userCfg = {}) {
      ...
      const medianOf = (arr) => {
        const xs = arr.filter(v => v !== "" && v != null && Number.isFinite(Number(v))).map(Number).sort((a,b)=>a-b);
        if (!xs.length) return "";
      ...
      console.log(`[${cfg.expId}] HOLD=${cfg.betweenRepTicks_hold} | PRIME(toggle)=${cfg.betweenRepTicks_toggle} | damp=${cfg.dampUsed}`);

      let cellCounter = 0;

      for (const dirTag of dirs) {
        for (const multB of cfg.multB_points) {
          // Normalize in case of float weirdness
          ...
          for (const primeCount of cfg.primeCounts) {
            ...
            for (let rep = 1; rep <= cfg.repsPerCell; rep++) {
              const r = await runOneTrial(
                phy, cfg, pressCUsed, dirTag, cellId, rep, multBUsed, baseB, ampD, edgeIdx,
                baselineModeUsed, modeLabel, primeCount,
              ...
              // capture ticks are inside summary rows; we’ll re-derive medians from summary post-hoc normally,
              // but to keep this script self-contained, we store placeholders here and compute medians as blank.
              // (If you want, we can also compute capture ticks in-trial, but that duplicates work.)
              cap114_list.push(NaN);
              cap136_list.push(NaN);

              await relax(phy, cfg, pressCUsed, cfg.betweenRepTicks_hold);
            }

            if (abortedAny) {
              console.warn(`[${cfg.expId}] ABORTED in cell ${cellId}`);
              // Still write a row with what we have
            }
            ...
            // capture medians intentionally left blank here (computed best from MASTER_summary later)
            const med_cap114 = "";
            const med_cap136 = "";
            ...
            console.log(
              `[${cfg.expId}] ${cellId} | p114_t8=${p114_t8.toFixed(4)} (114=${n114_t8}/${n}) | p114_peak=${p114_peak.toFixed(4)}`
            );
          }
        }
      }
  ...
  window.solPhase311_16bn4_parityTruthTableAtCliff_v1 = Runner;
  console.log("✅ Installed: solPhase311_16bn4_parityTruthTableAtCliff_v1");
  await Runner.run();
