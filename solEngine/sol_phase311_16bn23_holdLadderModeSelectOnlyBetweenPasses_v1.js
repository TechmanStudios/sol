// Phase 3.11 — 16BN23_holdLadderModeSelectOnlyBetweenPasses_v1
// Goal: ablate RESET to test whether ModeSelect alone resets cross-pass carryover.
//
// Protocol (per dir):
...
// - PASS1: DESC (800->0)
// - BETWEEN: ModeSelect ONLY (no precondition, no baseline restore, no washout)
// - PASS2: ASC (0->800)
...
(async () => {
  ...
  const CFG = {
    expIdBase: "sol_phase311_16bn23_holdLadderModeSelectOnlyBetweenPasses_v1",
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[BN23] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[BN23] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[BN23] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[BN23] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  const runAll = async (phy, pressCUsed) => {
    ...
    console.log("\n[BN23] START", runTag);
    console.log("[BN23] Between-pass action: MODESELECT ONLY");
    ...
    try {
      for (const dirTag of CFG.dirs) {
        console.log(`\n[BN23] DIR=${dirTag} | baseline restore + modeSelect + precondition (one-time)`);
        ...
        for (let pi = 0; pi < passes.length; pi++) {
          ...
          console.log(`\n[BN23] PASS ${pass.passLabel} begins for DIR=${dirTag}`);
          ...
          for (let ci = 0; ci < pass.order.length; ci++) {
            ...
            console.log(`[BN23]  Condition ${conditionIndex}/${pass.order.length} hold=${holdTicks} reps=${CFG.repsPerCondition}`);
            ...
            for (let rep = 1; rep <= CFG.repsPerCondition; rep++) {
              ...
              if (r.aborted) throw new Error("[BN23] Aborted due to non-finite values.");
            }
          }

          // Between pass1 and pass2: ModeSelect only
          if (pi === 0) {
            console.log("\n[BN23] BETWEEN PASSES: ModeSelect ONLY (no precondition, no washout)");
            await modeSelectOnce(phy, pressCUsed);
          }
        }
      }
