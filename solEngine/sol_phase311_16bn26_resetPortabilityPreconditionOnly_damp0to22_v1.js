// Phase 3.11 — 16BN26_resetPortabilityPreconditionOnly_damp0to22_v1
// Goal: Test portability of RESET opcode (Precondition-only) across dampUsed = 0..22.
//
...
// - PASS1: DESC (800->0)
// - BETWEEN PASSES: Precondition ONLY (no modeSelect, no baseline restore, no washout)
// - PASS2: ASC (0->800)
// - no baseline restore between holds or passes
//
// Exports:
...
(async () => {
  ...
  const CFG = {
    expIdBase: "sol_phase311_16bn26_resetPortabilityPreconditionOnly_damp0to22_v1",
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[BN26] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[BN26] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[BN26] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[BN26] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  const runAll = async (phy, pressCUsed) => {
    ...
    console.log("\n[BN26] START", runTag);
    console.log("[BN26] damp sweep:", CFG.dampList.join(","));
    console.log("[BN26] between-pass action: PRECONDITION ONLY");
    console.log("[BN26] trace sampling: stride=", CFG.traceStride, "repsPerCond=", CFG.traceRepsPerCondition);
    ...
    try {
      for (const dampUsed of CFG.dampList) {
        console.log(`\n[BN26] === dampUsed=${dampUsed} ===`);

        for (const dirTag of CFG.dirs) {
          console.log(`[BN26] DIR=${dirTag} | baseline restore + modeSelect + precondition (initial)`);
          ...
          {
            const pass = passes[0];
            console.log(`[BN26] PASS ${pass.passLabel} | holds=${pass.order.join(",")} | reps=${CFG.repsPerCondition}`);
          ...
          // BETWEEN PASSES: Precondition ONLY
          console.log(`[BN26] BETWEEN PASSES for DIR=${dirTag}, damp=${dampUsed}: Precondition ONLY`);
          await runPreconditionOnce(phy, pressCUsed, dampUsed, dirTag, baseB, ampD);
