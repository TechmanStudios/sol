/* Phase 3.11 — 16BA: Early-lock Hysteresis (preconditioned, continuous)
   Focus: tick-8 arbitration boundary; includes 1.00+ on the DOWN start.
*/
(() => {
  "use strict";

  const R = window.solPhase311_16az_lateBoundaryHysteresisZoom_v1
    ? JSON.parse(JSON.stringify(window.solPhase311_16az_lateBoundaryHysteresisZoom_v1))
    : null;

  // If 16AZ isn't loaded, we embed a minimal full runner (same as 16AZ core).
  // For simplicity + copy/paste safety, this script is standalone (no dependency).

  const S = {
    version: "sol_phase311_16ba_earlyLockHysteresis_v1",
    _stop: false,
    ...
    cfg: {
      ...
      // Early-lock band (includes 1.00+)
      upList:   [0.970,0.975,0.980,0.985,0.990,0.995,1.000,1.010,1.020],
      downList: [1.020,1.010,1.000,0.995,0.990,0.985,0.980,0.975,0.970],
      ...
      label: "B2_d20_earlyLock_hysteresis"
    },
    ...
    async _waitForPhysics(timeoutMs = 15000, pollMs = 50){
      ...
      throw new Error("[16ba] timed out waiting for physics.");
    },

    _freezeLiveLoop(){
      const app = this._getApp();
      if (!app?.config) throw new Error("[16ba] App not ready.");
    ...
    _inject(phy, id, amt){
      ...
      if (!n) throw new Error(`[16ba] node not found: ${id}`);
    ...
    async run(userCfg = {}){
      ...
      console.log(`[16ba] START ${this.version} @ ${startTag} | pressC=${pressCUsed} | damp=${g.dampUsed}`);

      const baselineMode = await this._baselineRestoreOnce(phy);
      console.log(`[16ba] baselineMode=${baselineMode} (restore ONCE; then continuous per path)`);
      ...
      console.log(`[16ba] DONE @ ${endTag}\n- ${baseName}_MASTER_summary.csv\n- ${baseName}_MASTER_busTrace.csv`);
      return { baseName, pressCUsed, dampUsed: g.dampUsed, stopped: this._stop };
    }
  };

  window.solPhase311_16ba_earlyLockHysteresis_v1 = S;
  console.log(`✅ solPhase311_16ba_earlyLockHysteresis_v1 installed.\nRun: await solPhase311_16ba_earlyLockHysteresis_v1.run()`);
})();
