/* Phase 3.11.16j — SimulPulse RatioMap v1

Goal:
  Map simultaneous (same-tick) dual-port injection as a 2D amplitude surface:
    - Inject into 114 and 136 on tick 0 (same cycle).
    - Sweep ampB (114) and ampD (136).
    - Measure whether each bus "turns on" (crosses abs flux threshold),
      and capture onset/peak timing + peak edge (filament capture).

Design (tuned for ~20–30 min):
  - ampB_list: [6, 10, 14, 18]
  - ampD_list: [6, 10, 14, 18]
  - repsPerCombo: 12
  - total combos: 4*4*12 = 192 runs
  - ticks per run: postTicks+1 = 37
  - everyMs=200 => ~7.4s/run => ~23.7 min total
...
Run:
  solPhase311_16j.runPack()

Stop:
  solPhase311_16j.stop()
...
(() => {
  ...
  const solPhase311_16j = {
    version: "3.11.16j_simulPulseRatioMap_v1",
    _stopFlag: false,
    stop() { this._stopFlag = true; console.warn("🛑 solPhase311_16j.stop(): stop flag set."); },
    ...
    cfg: {
      ...
      // simultaneous pulse amplitude grid
      ampB_list: [6, 10, 14, 18],   // injection into 114
      ampD_list: [6, 10, 14, 18],   // injection into 136
      repsPerCombo: 12,

      postTicks: 36,               // observe after injection
      dt: 0.12,
      everyMs: 200,
      spinWaitMs: 1.5,
      ...
      filenameBase: "sol_phase311_16j_simulPulseRatioMap_v1",
    },
    ...
    async _waitForPhysics({ timeoutMs = 15000, pollMs = 50 } = {}) {
      ...
      throw new Error("solPhase311_16j: timed out waiting for physics.");
    },
    ...
    _freezeLiveLoop() {
      const app = this._getApp();
      if (!app?.config) throw new Error("solPhase311_16j: App not ready.");
    ...
    _injectById(phy, id, amount) {
      ...
      if (!n) throw new Error(`solPhase311_16j: injector node not found: ${id}`);
    ...
    async _runOne({ runIndex, repIndex, wantId, pressC, baseDamp, ampB, ampD }) {
      ...
      // post-injection tracking (injection is at tick 0 for both)
      const bus114 = { onsetTick: null, peakAbs: -1, peakTick: null, peakF89: 0, peakF79: 0 };
      const bus136 = { onsetTick: null, peakAbs: -1, peakTick: null, peakF89: 0, peakF79: 0 };
      ...
      const met = await this._metronomeFactory({
        ...
        onTick: async ({ tick, tMs, lateByMs }) => {
          if (tick === 0) {
            // atomic-ish simultaneous: both rhos are incremented before the step
          ...
          if (a114 > bus114.peakAbs) {
            bus114.peakAbs = a114; bus114.peakTick = tick; bus114.peakF89 = f114_89; bus114.peakF79 = f114_79;
          }
          if (a136 > bus136.peakAbs) {
            bus136.peakAbs = a136; bus136.peakTick = tick; bus136.peakF89 = f136_89; bus136.peakF79 = f136_79;
          }
    ...
    async runPack(userCfg = {}) {
      ...
      const app = this._getApp();
      if (!app) throw new Error("solPhase311_16j: SOLDashboard not found.");
      ...
      console.log(`🧪 Phase 3.11.16j starting (${this.version})`);
      ...
      console.log("✅ Phase 3.11.16j complete:", base2);
      return { baseName: base2, runsPlanned: plan.length, stopped: this._stopFlag };
    }
  };

  window.solPhase311_16j = solPhase311_16j;
  console.log(`✅ solPhase311_16j installed (${solPhase311_16j.version}). Run: solPhase311_16j.runPack()`);
