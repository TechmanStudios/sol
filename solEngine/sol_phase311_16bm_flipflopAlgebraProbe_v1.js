// Phase 3.11 — 16BM (flip-flop algebra test): HOLD-mode latch reversibility
// Goal: test whether the "toggle prime" behaves like a T-flip-flop:
//   M0: holdOnly (no toggle prime)                 -> expected default phase
//   M1: toggleThenHold (1 toggle prime)            -> expected flipped phase (like 16BL BL1)
//   M2: doubleToggleThenHold (2 toggle primes)     -> expected return to default phase (if true T-flip-flop)
//
// Run at TWO probe points (fast, high signal):
//   - multB = 1.0125 (near x50 band)
//   - multB = 1.0130 (near x50 / slightly above)
// For each probe and each mode, run 120 HOLD reps.
// Keep: damp=20, segmentTicks=121, HOLD betweenRepTicks=81.
//
// Exports:
//   - phaseFlipflop_probe.csv  (compact verdict table)
//   - MASTER_summary.csv       (full per-rep metrics)
//   - MASTER_busTrace.csv      (optional; kept ON for audit)
...
(async () => {
  ...
  const CFG = {
    expId: "sol_phase311_16bm_flipflopAlgebraProbe_v1",
    ...
    // Probe points
    probeMultBs: [1.0125, 1.0130],

    // How many HOLD reps to measure after init
    repsPerProbe: 120,

    // Init modes
    modes: [
      { id: "M0_holdOnly", toggles: 0 },
      { id: "M1_toggle1",  toggles: 1 },
      { id: "M2_toggle2",  toggles: 2 },
    ],

    // Directional precondition (use "down" because it's more turbulence-stressy historically)
    dir: "down",
    preLow_multB: 0.90,
    preLow_ticks: 320,
    preHigh_multB: 1.08,
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[16BM] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[16BM] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[16BM] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[16BM] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  const runPrecondition = async (phy, cfg, pressCUsed, baseB, ampD) => {
    const multBUsed = (cfg.dir === "up") ? cfg.preLow_multB : cfg.preHigh_multB;
    const ticks = (cfg.dir === "up") ? cfg.preLow_ticks : cfg.preHigh_ticks;
  ...
  const relax = async (phy, cfg, pressCUsed, nTicks) => {
    for (let k = 0; k < nTicks; k++) phy.step(cfg.dt, pressCUsed, cfg.dampUsed);
  };

  // ----------------------------
  // Trial (single rep) — returns tick8 winner + capture ticks
  // ----------------------------
  const runOneRep = async (phy, cfg, pressCUsed, multBUsed, baseB, ampD, edgeIdx, modeId, rep, recordTrace, summaryLines, traceLines, runTag) => {
    ...
    for (let tick = 0; tick < cfg.segmentTicks; tick++) {
      ...
      if (recordTrace) {
        traceLines.push(csvRow([
          cfg.expId, runTag, modeId, cfg.dir, multBUsed, rep, tick,
    ...
    summaryLines.push(csvRow([
      cfg.expId, runTag, modeId, cfg.dir, multBUsed, rep,
    ...
    return { aborted, winner_t8, cap114, cap136, t8_abs114, t8_abs136, winner_peakBus };
  };
  ...
  const Runner = {
    async run(userCfg = {}) {
      ...
      const summaryHeader = [
        "schema","runTag","mode","dir","multB","rep",
      ...
      const traceHeader = [
        "schema","runTag","mode","dir","multB","rep","tick",
      ...
      const probeHeader = [
        "schema","runTag","mode","multB","n",
        "p114_t8","p136_t8","pTie_t8",
        "median_captureTick_114","median_captureTick_136"
      ];

      const summaryLines = [csvRow(summaryHeader)];
      const traceLines = [csvRow(traceHeader)];
      const probeLines = [csvRow(probeHeader)];

      const probeAgg = []; // {mode,multB, winners[], cap114s[], cap136s[]}

      console.log(`\n[${cfg.expId}] START @ ${startTag}`);
      console.log(`[${cfg.expId}] Probes: ${cfg.probeMultBs.join(", ")} | reps=${cfg.repsPerProbe}`);
      console.log(`[${cfg.expId}] Modes: ${cfg.modes.map(m=>m.id).join(", ")}`);
      console.log(`[${cfg.expId}] HOLD=${cfg.betweenRepTicks_hold} (even stride). TogglePrime=${cfg.betweenRepTicks_toggle} (odd stride).`);

      // For each probe multB, for each mode:
      for (const multBUsed of cfg.probeMultBs) {
        for (const m of cfg.modes) {
          // baseline restore + mode select + precondition (once per probe/mode)
          const baselineModeUsed = (window.SOLBaseline?.restore) ? "SOLBaseline.restore" : "internal_snapshot_restored";
          // do the actual restore
          if (window.SOLBaseline?.restore) await window.SOLBaseline.restore();
          else {
            // internal snapshot fallback
            if (!Runner._snap) Runner._snap = makeInternalSnapshot(phy);
            restoreInternalSnapshot(phy, Runner._snap);
          }

          await modeSelectOnce(phy, cfg, pressCUsed, cfg.dampUsed);
          await runPrecondition(phy, cfg, pressCUsed, baseB, ampD);

          // Prime toggles: each prime = one toggle-stride rep (NOT recorded to exports for cleanliness)
          for (let t = 0; t < m.toggles; t++) {
            await runOneRep(
              phy, cfg, pressCUsed, multBUsed, baseB, ampD, edgeIdx,
              `${m.id}_PRIME${t+1}`, 0, false,
              // record summary for primes? no (keep clean)
              [], [], runTag
            );
          ...
          // HOLD reps
          for (let rep = 1; rep <= cfg.repsPerProbe; rep++) {
            const r = await runOneRep(
              phy, cfg, pressCUsed, multBUsed, baseB, ampD, edgeIdx,
              m.id, rep, cfg.exportBusTrace,
              summaryLines, traceLines, runTag
            );
            winners.push(r.winner_t8);
            if (r.cap114 !== "") cap114s.push(Number(r.cap114));
            if (r.cap136 !== "") cap136s.push(Number(r.cap136));
            await relax(phy, cfg, pressCUsed, cfg.betweenRepTicks_hold);
          }

          probeAgg.push({ mode: m.id, multB: multBUsed, winners, cap114s, cap136s, baselineModeUsed });
        }
      }

      // Build compact probe table
      const median = (arr) => {
      ...
      for (const row of probeAgg) {
        const n = row.winners.length || 1;
        const n114 = row.winners.filter(x => x === 114).length;
        const n136 = row.winners.filter(x => x === 136).length;
        const nTie = row.winners.filter(x => x === 0).length;
        probeLines.push(csvRow([
          cfg.expId, runTag, row.mode, row.multB, n,
          n114 / n, n136 / n, nTie / n,
          median(row.cap114s), median(row.cap136s)
        ]));
      }
      ...
      downloadText(`${baseName}_phaseFlipflop_probe.csv`, probeLines.join(""));
      downloadText(`${baseName}_MASTER_summary.csv`, summaryLines.join(""));
      if (cfg.exportBusTrace) downloadText(`${baseName}_MASTER_busTrace.csv`, traceLines.join(""));

      console.log(`\n[${cfg.expId}] DONE @ ${endTag}`);
      console.log(`- ${baseName}_phaseFlipflop_probe.csv`);
      console.log(`- ${baseName}_MASTER_summary.csv`);
      if (cfg.exportBusTrace) console.log(`- ${baseName}_MASTER_busTrace.csv`);

      return { expId: cfg.expId, baseName, pressCUsed, dampUsed: cfg.dampUsed };
    }
  };

  window.solPhase311_16bm_flipflopAlgebraProbe_v1 = Runner;
  console.log("✅ Installed: solPhase311_16bm_flipflopAlgebraProbe_v1");
  await Runner.run();
