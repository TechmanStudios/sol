// Phase 3.11 — 16BN24_holdLadderPreconditionOnlyBetweenPasses_v1
// Goal: ablate RESET to test whether Precondition alone resets cross-pass carryover.
//
// Protocol (per dir):
...
// - PASS1: DESC (800->0)
// - BETWEEN: Precondition ONLY (no modeSelect, no baseline restore, no washout)
// - PASS2: ASC (0->800)
...
(async () => {
  ...
  const CFG = {
    expIdBase: "sol_phase311_16bn24_holdLadderPreconditionOnlyBetweenPasses_v1",
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[BN24] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[BN24] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[BN24] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[BN24] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  const runAll = async (phy, pressCUsed) => {
    ...
    console.log("\n[BN24] START", runTag);
    console.log("[BN24] Between-pass action: PRECONDITION ONLY");
    ...
    try {
      for (const dirTag of CFG.dirs) {
        console.log(`\n[BN24] DIR=${dirTag} | baseline restore + modeSelect + precondition (one-time)`);
        ...
        for (let pi = 0; pi < passes.length; pi++) {
          ...
          console.log(`\n[BN24] PASS ${pass.passLabel} begins for DIR=${dirTag}`);
          ...
          for (let ci = 0; ci < pass.order.length; ci++) {
            ...
            console.log(`[BN24]  Condition ${conditionIndex}/${pass.order.length} hold=${holdTicks} reps=${CFG.repsPerCondition}`);
            ...
            for (let rep = 1; rep <= CFG.repsPerCondition; rep++) {
              ...
              if (r.aborted) throw new Error("[BN24] Aborted due to non-finite values.");
            }
          }

          // Between pass1 and pass2: Precondition only
          if (pi === 0) {
            console.log("\n[BN24] BETWEEN PASSES: Precondition ONLY (no modeSelect, no washout)");
            await runPreconditionOnce(phy, pressCUsed, dirTag, baseB, ampD);
          }
        }
      }
