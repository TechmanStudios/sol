// Phase 3.11 — 16BN20_holdLadderWashoutLong_v1
// Goal: test whether cross-pass carryover decays with a longer neutral washout (damp=20).
//
// Protocol:
...
(async () => {
  ...
  const CFG = {
    expIdBase: "sol_phase311_16bn20_holdLadderWashoutLong_v1",
    ...
    // KEY VARIABLE: longer washout between passes
    washoutBetweenPassTicks: 16000,
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[BN20] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[BN20] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[BN20] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[BN20] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  const runAll = async (phy, pressCUsed) => {
    ...
    console.log("\n[BN20] START", runTag);
    console.log("[BN20] washoutBetweenPassTicks =", CFG.washoutBetweenPassTicks);
    ...
    try {
      for (const dirTag of CFG.dirs) {
        console.log(`\n[BN20] DIR=${dirTag} | baseline restore + modeSelect + precondition (one-time)`);
        ...
        for (let pi = 0; pi < passes.length; pi++) {
          ...
          console.log(`\n[BN20] PASS ${pass.passLabel} begins for DIR=${dirTag}`);
          ...
          for (let ci = 0; ci < pass.order.length; ci++) {
            ...
            console.log(`[BN20]  Condition ${conditionIndex}/${pass.order.length} hold=${holdTicks} reps=${CFG.repsPerCondition}`);
            ...
            for (let rep = 1; rep <= CFG.repsPerCondition; rep++) {
              ...
              if (r.aborted) throw new Error("[BN20] Aborted due to non-finite values.");
            }
          }

          if (pi === 0 && CFG.washoutBetweenPassTicks > 0) {
            console.log(`\n[BN20] WASHOUT between passes: ${CFG.washoutBetweenPassTicks} ticks (no injections)`);
