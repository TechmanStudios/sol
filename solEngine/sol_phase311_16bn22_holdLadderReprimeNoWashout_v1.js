// Phase 3.11 — 16BN22_holdLadderReprimeNoWashout_v1
// Goal: test whether REPRIME (modeSelect + precondition) alone resets carryover,
// without any neutral washout between passes.
//
// Protocol (per dir):
...
// - PASS1: DESC (800->0)
// - REPRIME immediately (modeSelect + precondition), NO washout
// - PASS2: ASC (0->800)
...
(async () => {
  ...
  const CFG = {
    expIdBase: "sol_phase311_16bn22_holdLadderReprimeNoWashout_v1",
    ...
    // Between passes
    washoutBetweenPassTicks: 0,      // <-- NO washout
    reprimeBetweenPasses: true,      // <-- REPRIME ON
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[BN22] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[BN22] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[BN22] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[BN22] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  const runAll = async (phy, pressCUsed) => {
    ...
    console.log("\n[BN22] START", runTag);
    console.log("[BN22] washoutBetweenPassTicks =", CFG.washoutBetweenPassTicks);
    console.log("[BN22] reprimeBetweenPasses =", CFG.reprimeBetweenPasses);
    ...
    try {
      for (const dirTag of CFG.dirs) {
        console.log(`\n[BN22] DIR=${dirTag} | baseline restore + modeSelect + precondition (one-time)`);
        ...
        for (let pi = 0; pi < passes.length; pi++) {
          ...
          console.log(`\n[BN22] PASS ${pass.passLabel} begins for DIR=${dirTag}`);
          ...
          for (let ci = 0; ci < pass.order.length; ci++) {
            ...
            console.log(`[BN22]  Condition ${conditionIndex}/${pass.order.length} hold=${holdTicks} reps=${CFG.repsPerCondition}`);
            ...
            for (let rep = 1; rep <= CFG.repsPerCondition; rep++) {
              ...
              if (r.aborted) throw new Error("[BN22] Aborted due to non-finite values.");
            }
          }

          // Between pass1 and pass2: reprime only (NO washout)
          if (pi === 0 && CFG.reprimeBetweenPasses) {
            console.log("\n[BN22] REPRIME between passes: modeSelect + precondition (no baseline restore, no washout)");
            await modeSelectOnce(phy, pressCUsed);
            await runPreconditionOnce(phy, pressCUsed, dirTag, baseB, ampD);
          }
        }
