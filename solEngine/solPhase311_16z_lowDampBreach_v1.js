/* Phase 3.11.16z — Low-Damp Breach Test (Adaptive Handshake)
   Goal:
     Intentionally "break" the clean 16Y packet by pushing damping lower and watching:
       - arbitration gets late or never happens
       - handshake fails to apply
       - precedence flips (114_first)
       - packet stops being "fast follow"
       - overall behavior becomes unstable / non-finite

   Same protocol as 16Y (for comparability):
     - multB=1.144, baseAmpB=4, baseAmpD=5.75, gain=22
     - offset=+1 => 136 at tick0, 114 at tick1
     - nudgeMult=0.20 (arbiterTick+1, only if 136 won)

   Sweep (low damp extension):
     - dampUsedList: [0.5, 1, 2, 3, 4]
     - repsPerDamp: 12

   Runtime note:
     - totalTicks bumped to 101 (still ~same total wall time as 16Y, but gives low-damp time to form/decay)

   Run:
     await solPhase311_16z_lowDampBreach_v1.run()

   Stop:
     solPhase311_16z_lowDampBreach_v1.stop()
...
(() => {
  ...
  const T = {
    version: "sol_phase311_16z_lowDampBreach_v1",
    _stop: false,
    ...
    cfg: {
      ...
      // PressC base (null => read from invariants/UI/fallback)
      pressCBase: null,

      // Damping sweep (explicit)
      dampUsedList: [0.5, 1, 2, 3, 4],
      ...
      // Safety (log + abort run if physics goes non-finite)
    ...
    async _waitForPhysics(timeoutMs = 15000, pollMs = 50) {
      ...
      throw new Error("[16z] timed out waiting for physics.");
    },

    _freezeLiveLoop() {
      const app = this._getApp();
      if (!app?.config) throw new Error("[16z] App not ready.");
    ...
    _inject(phy, id, amt) {
      ...
      if (!n) throw new Error(`[16z] node not found: ${id}`);
    ...
    async run(userCfg = {}) {
      ...
      // Stitch edge (your tertiary link)
      ...
      const summaryHeader = [
        ...
        // 16Z extras (so we can tell if low damp went non-finite)
      ...
      console.log(
        `[16z] plan=${plan.length} | pressCBase=${pressCBase} | dampUsedList=${this.cfg.dampUsedList.join("/")}` +
        ` | protocol: multB=${this.cfg.multB} ratioBD=${ratioBD.toFixed(6)} offset=${this.cfg.offset} nudgeMult=${this.cfg.nudgeMult}` +
        ` | ticks=${this.cfg.totalTicks} everyMs=${this.cfg.everyMs}`
      );
      ...
      for (const item of plan) {
        ...
        for (let tick = 0; tick < this.cfg.totalTicks; tick++) {
          ...
          // Non-finite guard (important at low damping)
          if (this.cfg.abortOnNonFinite && !this._isFiniteEdgeSample(top2, f114_89, f114_79, f136_89, f136_79, f89_79)) {
            ...
            console.warn(`[16z] ABORT runIndex=${runIndex} damp=${dampUsed} rep=${repIndex} at tick=${tick}: non-finite flux sample.`);
            break;
          }
        ...
        if (runIndex % 20 === 0) {
          console.log(
            `[16z] progress ${runIndex}/${plan.length} | damp=${Number(dampUsed).toFixed(2)} | arb=${arbiter_owner}@${arbiter_tick} hs=${handshake_tick}` +
            ` | prec=${precedence} class=${packetClass} Δ=${deltaTicks} | stitchPeak=${cross_peakAbs_89_79.toExponential(3)} | status=${runStatus}`
          );
        }
      }
      ...
      console.log(`✅ [16z] DONE. Upload:\n- ${baseName}_MASTER_summary.csv\n- ${baseName}_MASTER_busTrace.csv`);
      return { baseName, runs: runIndex, stopped: this._stop, pressCUsed: pressCBase, dampUsedList: this.cfg.dampUsedList };
    }
  };

  window.solPhase311_16z_lowDampBreach_v1 = T;
  console.log(`✅ solPhase311_16z_lowDampBreach_v1 installed (${T.version}). Run: await solPhase311_16z_lowDampBreach_v1.run()`);
})();
