// Phase 3.11 — 16BN31_modeSelectStrengthSweep_damp21_hold160_v1
// Target: damp21, ASC hold160, after a strong DESC(800).
//
...
//
//   S1..S5  RESET_MODESELECT_STRENGTH_SWEEP:
//      baseline -> modeSelect(full) -> precondition -> DESC(800)
//      -> modeSelect(strength_i) -> precondition -> ASC(160)
//
//   D  RESET_FULL_REPRIME_REFERENCE:
//      same as S5 (15 blocks, 120 inject), kept as a named reference.
//
// Exports:
//   *_MASTER_summary.csv
...
(async () => {
  ...
  const CFG = {
    expIdBase: "sol_phase311_16bn31_modeSelectStrengthSweep_damp21_hold160_v1",
    ...
    // busTrace sampling (keep files small, still captures shape)
    traceStride: 4,
    ...
    // Mode select defaults (full)
    wantId: 82,
    injectorIds: [90, 82],
    dreamBlockSteps: 2,
    finalWriteMult: 1,

    // Strength sweep table (between-pass modeSelect only)
    // You can tune these later; this gives a nice log-ish ramp.
    modeSelectStrengths: [
      { seqId: "S1", seqLabel: "RESET_MS_04B_30A", dreamBlocks: 4,  injectAmount: 30 },
      { seqId: "S2", seqLabel: "RESET_MS_06B_45A", dreamBlocks: 6,  injectAmount: 45 },
      { seqId: "S3", seqLabel: "RESET_MS_08B_60A", dreamBlocks: 8,  injectAmount: 60 },
      { seqId: "S4", seqLabel: "RESET_MS_10B_80A", dreamBlocks: 10, injectAmount: 80 },
      { seqId: "S5", seqLabel: "RESET_MS_12B_100A", dreamBlocks: 12, injectAmount: 100 }
    ],

    // Full reference (also used for initial modeSelect)
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[BN31] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[BN31] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[BN31] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[BN31] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  const runAll = async (phy, pressCUsed) => {
    ...
    const sequences = [
      ...
      ...CFG.modeSelectStrengths.map(s => ({ seqId: s.seqId, seqLabel: s.seqLabel, between: "MS_PRE", strength: s })),
      { seqId: "D", seqLabel: "RESET_FULL_REPRIME_REF", between: "MS_PRE", strength: { dreamBlocks: 15, injectAmount: 120 } }
    ];

    console.log("\n[BN31] START", runTag);
    console.log("[BN31] sequences:", sequences.map(s => s.seqId).join(", "));

    try {
      for (const dampUsed of CFG.dampList) {
        console.log(`\n[BN31] === dampUsed=${dampUsed} ===`);
        for (const dirTag of CFG.dirs) {
          console.log(`[BN31] DIR=${dirTag}`);

          for (const seq of sequences) {
            const ctx = { _snap: null };
            const baselineModeUsed = await baselineRestore(phy, ctx);

            // Initial canonical setup is always full modeSelect + precondition
            ...
            if (seq.seqId === "C") {
              console.log(`[BN31]  C ASC_ONLY_CONTROL | ASC hold160 reps=${CFG.repsPerCondition}`);
              await runASC();
              continue;
            }

            // DESC
            console.log(`[BN31]  ${seq.seqId} ${seq.seqLabel} | DESC hold800 reps=${CFG.repsPerCondition}`);
            for (let rep = 1; rep <= CFG.repsPerCondition; rep++) {
              globalRepIndex++;
              await runOneRep(
                phy, pressCUsed, dampUsed,
            ...
            if (seq.between === "PREONLY") {
              console.log(`[BN31]   BETWEEN: precondition-only`);
              await runPreconditionOnce(phy, pressCUsed, dampUsed, dirTag, baseB, ampD);
            } else if (seq.between === "MS_PRE") {
              const st = seq.strength || CFG.fullModeSelect;
              console.log(`[BN31]   BETWEEN: modeSelect(dreamBlocks=${st.dreamBlocks}, injectAmount=${st.injectAmount}) + precondition`);
              await modeSelectOnce(phy, pressCUsed, dampUsed, st);
              await runPreconditionOnce(phy, pressCUsed, dampUsed, dirTag, baseB, ampD);
            } else {
              console.log(`[BN31]   BETWEEN: none`);
            }

            // ASC
            console.log(`[BN31]  ASC hold160 reps=${CFG.repsPerCondition}`);
            await runASC();
          }
        }
      }
