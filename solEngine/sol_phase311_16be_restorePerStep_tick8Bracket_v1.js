// Phase 3.11 — 16BE (restore-per-step) tick8 bracket sweep
// Goal: find where tick8 dominance (p114_t8) lifts off 0 and crosses 0.5 under the SAME protocol as 16BD.
// UI-neutral: no camera/graph motion calls. Injects by direct node rho only.
// Exports: MASTER_summary + MASTER_busTrace + rates_by_dir_multB + threshold_estimates + turbulence_profile
...
(async () => {
  ...
  // ----------------------------
  const CFG = {
    expId: "sol_phase311_16be_restorePerStep_tick8Bracket_v1",

    // Bracket sweep (coarse). If you find the crossing, you can do a follow-up fine scan.
    // 0.958 → 0.990 in 0.002 steps
    upList:   [0.958,0.960,0.962,0.964,0.966,0.968,0.970,0.972,0.974,0.976,0.978,0.980,0.982,0.984,0.986,0.988,0.990],
    downList: [0.990,0.988,0.986,0.984,0.982,0.980,0.978,0.976,0.974,0.972,0.970,0.968,0.966,0.964,0.962,0.960,0.958],
    ...
    // Fixed regime
    dt: 0.12,
    pressCBase: null,     // null => use invariants pressC or UI slider or fallback 2.0
    dampUsed: 20,
    markerTick: 8,

    // Directional precondition (applied ONCE per step)
    preLow_multB: 0.90,
    preLow_ticks: 320,
    preHigh_multB: 1.08,
    preHigh_ticks: 320,
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[16BE] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[16BE] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[16BE] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    try {
      await window.SOLBaseline.ensure({ force: false, exportJson: false });
    } catch (e) {
      console.warn("[16BE] SOLBaseline.ensure failed (continuing):", e);
    }
  };
  ...
  // ----------------------------
  // 3) DERIVED TABLE BUILDERS
  // ----------------------------
  const buildRatesByDirMultB = (rows) => {
    const m = new Map(); // key dir|multB
    for (const r of rows) {
  ...
  const estimateX50 = (pts) => {
    ...
    for (let i = 0; i < pts.length - 1; i++) {
      ...
      if (s0 === 0) return { x50: x0, bracket: [x0, x0], pBracket: [p0, p0] };
      if (s0 * s1 < 0) {
        const t = (0.5 - p0) / (p1 - p0);
        const x50 = x0 + t * (x1 - x0);
        return { x50, bracket: [x0, x1], pBracket: [p0, p1] };
      }
    }
  ...
  const buildThresholdEstimates = (rates) => {
    ...
    const metrics = [
      { name: "peakBus", key: "p114_peak" },
      { name: "tick8", key: "p114_t8" }
    ];
    ...
    for (const met of metrics) {
      ...
      if (estByDir.up && estByDir.down && estByDir.up.x50 !== "" && estByDir.down.x50 !== "") {
  ...
  const buildTurbulenceProfile = (rates) => {
    ...
    for (const d of dirs) {
      ...
      const dp = (arr, i, key) => {
        if (n < 2) return "";
        if (i === 0) {
          const dx = arr[1].multB - arr[0].multB;
          return dx ? (arr[1][key] - arr[0][key]) / dx : "";
        }
        if (i === n - 1) {
          const dx = arr[n-1].multB - arr[n-2].multB;
          return dx ? (arr[n-1][key] - arr[n-2][key]) / dx : "";
        }
        const dx = arr[i+1].multB - arr[i-1].multB;
        return dx ? (arr[i+1][key] - arr[i-1][key]) / dx : "";
      };

      for (let i = 0; i < n; i++) {
        const r = pts[i];
        const pP = r.p114_peak;
        const p8 = r.p114_t8;
        out.push({
          dir: d,
          multB: r.multB,
  ...
  const Runner = {
    ...
    async run(userCfg = {}) {
      ...
      console.log(`[${cfg.expId}] MODE = restore-per-step | pressCUsed=${pressCUsed} dampUsed=${cfg.dampUsed} dt=${cfg.dt}`);
      console.log(`[${cfg.expId}] Bracket sweep up=[${cfg.upList[0]}..${cfg.upList[cfg.upList.length-1]}] stepCount=${cfg.upList.length} repsPerStep=${cfg.repsPerStep}`);
      ...
      const runOneTrial = async (dirTag, stepIndex, rep, multBUsed, baselineModeUsed) => {
        ...
        for (let rlx = 0; rlx < cfg.betweenRepTicks; rlx++) {
          if (this.stopFlag) break;
          phy.step(cfg.dt, pressCUsed, cfg.dampUsed);
        }
      ...
      const runStep = async (dirTag, stepIndex, multBUsed) => {
        if (this.stopFlag) return;

        // baseline restore ONCE PER STEP
        ...
        if (dirTag === "up") {
          await runPrecondition(cfg.preLow_multB, cfg.preLow_ticks);
        } else {
          await runPrecondition(cfg.preHigh_multB, cfg.preHigh_ticks);
        }

        // reps under same step-state
        for (let rep = 1; rep <= cfg.repsPerStep; rep++) {
          if (this.stopFlag) break;
          const r = await runOneTrial(dirTag, stepIndex, rep, multBUsed, baselineModeUsed);
          if (r?.aborted) break;
        }
      };

      const runPath = async (dirTag, list) => {
        for (let s = 0; s < list.length; s++) {
          if (this.stopFlag) break;
          await runStep(dirTag, s, list[s]);
        }
      };

      console.log(`[${cfg.expId}] UP path...`);
      await runPath("up", cfg.upList);

      console.log(`[${cfg.expId}] DOWN path...`);
      await runPath("down", cfg.downList);
  ...
  window.solPhase311_16be_restorePerStep_tick8Bracket_v1 = Runner;
  console.log("✅ Installed: solPhase311_16be_restorePerStep_tick8Bracket_v1");
  await Runner.run();
