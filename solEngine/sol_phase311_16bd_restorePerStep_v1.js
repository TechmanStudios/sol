// Phase 3.11 — Path 1: RESTORE-PER-STEP (Goldilocks reset)
// 16BD — matched band scan with baseline restore + modeSelect + precondition ONCE PER STEP
...
(async () => {
  ...
  // ----------------------------
  const CFG = {
    expId: "sol_phase311_16bd_restorePerStep_v1",

    // Band (same as 16BB/16BC for comparability)
    upList:   [0.9520,0.9525,0.9530,0.9535,0.9540,0.9545,0.9550,0.9555,0.9560,0.9565,0.9570,0.9575,0.9580],
    downList: [0.9580,0.9575,0.9570,0.9565,0.9560,0.9555,0.9550,0.9545,0.9540,0.9535,0.9530,0.9525,0.9520],

    repsPerStep: 30,      // increase for smoother curves
    segmentTicks: 121,
    betweenRepTicks: 80,  // hold after each rep (constant), within-step only
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[16BD] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[16BD] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[16BD] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    try {
      ...
      console.warn("[16BD] SOLBaseline.ensure failed (continuing):", e);
    }
  };
  ...
  const Runner = {
    ...
    async run(userCfg = {}) {
      ...
      console.log(`[${cfg.expId}] steps up=${cfg.upList.length} down=${cfg.downList.length} repsPerStep=${cfg.repsPerStep}`);
      ...
      const runOneTrial = async (dirTag, stepIndex, rep, multBUsed, baselineModeUsed) => {
        ...
        // constant hold after each rep (within-step)
      ...
      const runStep = async (dirTag, stepIndex, multBUsed) => {
        ...
        // KEY: baseline restore ONCE PER STEP
        ...
        // reps at this multB under identical step state
  ...
  // Install + run now
  window.solPhase311_16bd_restorePerStep_v1 = Runner;
  console.log("✅ Installed: solPhase311_16bd_restorePerStep_v1");
  await Runner.run();
