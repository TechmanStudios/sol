// Phase 3.11 — 16BN16_holdOrderSwapCounterbalance_v1
// Goal: counterbalance BN15 by running PASS B first, then PASS A (BA/AB) to separate order effects from pass-position drift
...
//
// Key design for the "order swap" counterbalance test:
// - For each dir: do ONE baseline restore + ONE modeSelect + ONE precondition at the very start.
// - Then run PASS B (orderB) FIRST, immediately followed by PASS A (orderA), with NO restore in between.
...
(async () => {
  ...
  // -----------------------------
  const CFG = {
    expIdBase: "sol_phase311_16bn16_holdOrderSwapCounterbalance_v1",
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[BN16] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[BN16] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[BN16] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[BN16] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  const runAll = async (phy, pressCUsed) => {
    ...
    console.log("\n[BN16] START", runTag);
    console.log("[BN16] orderA =", CFG.orderA.join(" -> "));
    console.log("[BN16] orderB =", CFG.orderB.join(" -> "));
    console.log("[BN16] Note: baseline restore is ONCE per dir; no restore between holds/passes.");
    console.log("[BN16] Note: PASS ORDER is B first, then A.");
    ...
    try {
      for (const dirTag of CFG.dirs) {
        console.log(`\n[BN16] DIR=${dirTag} | baseline restore + modeSelect + precondition (one-time)`);
        ...
        // Counterbalance: B then A
        const passes = [
          { passId: 1, passLabel: "B", order: CFG.orderB, orderLabel: "B_" + CFG.orderB.join("_") },
          { passId: 2, passLabel: "A", order: CFG.orderA, orderLabel: "A_" + CFG.orderA.join("_") }
        ];

        for (const pass of passes) {
          console.log(`\n[BN16] PASS ${pass.passLabel} begins for DIR=${dirTag} | order=${pass.order.join("->")}`);
          ...
          for (let ci = 0; ci < pass.order.length; ci++) {
            ...
            console.log(`[BN16]  Condition ${conditionIndex}/${pass.order.length} hold=${holdTicks} reps=${CFG.repsPerCondition}`);
            ...
            for (let rep = 1; rep <= CFG.repsPerCondition; rep++) {
              ...
              if (r.aborted) {
                console.warn(`[BN16] ABORT: dir=${dirTag} pass=${pass.passLabel} hold=${holdTicks} rep=${rep} (non-finite).`);
                throw new Error("[BN16] Aborted due to non-finite values.");
              }

              if (rep === 1 || rep === 2 || rep === 3 || rep === 5 || rep === 10 || rep === 20) {
                console.log(
                  `[BN16]   dir=${dirTag} pass=${pass.passLabel} hold=${holdTicks} rep=${rep} |`
