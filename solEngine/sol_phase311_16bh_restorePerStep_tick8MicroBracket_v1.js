// Phase 3.11 — 16BH (restore-per-step) tick8 micro-bracket below 0.987
// Purpose: force p114_t8 to dip < 0.5 so tick8 x50 becomes a true crossing (not “first point equals 0.5”).
// Protocol: identical to 16BG (restore-per-step, modeSelect once per step, directional precondition once per step).
...
(async () => {
  ...
  const CFG = {
    expId: "sol_phase311_16bh_restorePerStep_tick8MicroBracket_v1",

    // 0.9830 → 0.9880 in 0.0005 steps (11 points)
    upList:   [0.9830,0.9835,0.9840,0.9845,0.9850,0.9855,0.9860,0.9865,0.9870,0.9875,0.9880],
    downList: [0.9880,0.9875,0.9870,0.9865,0.9860,0.9855,0.9850,0.9845,0.9840,0.9835,0.9830],
    ...
    dt: 0.12,
    pressCBase: null,  // null => invariants/UI/fallback
    dampUsed: 20,
    markerTick: 8,

    // Directional precondition ONCE per step (same as 16BE/16BF/16BG)
    preLow_multB: 0.90,
    preLow_ticks: 320,
    preHigh_multB: 1.08,
    preHigh_ticks: 320,
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[16BH] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[16BH] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[16BH] node not found: ${id}`);
  ...
  // ----------------------------
  // Baseline restore (with snapshot fallback)
  // ----------------------------
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[16BH] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  const Runner = {
    ...
    async run(userCfg = {}) {
      ...
      console.log(`[${cfg.expId}] restore-per-step | pressCUsed=${pressCUsed} dampUsed=${cfg.dampUsed} dt=${cfg.dt}`);
      console.log(`[${cfg.expId}] repsPerStep=${cfg.repsPerStep} segmentTicks=${cfg.segmentTicks} betweenRepTicks=${cfg.betweenRepTicks}`);
      console.log(`[${cfg.expId}] UP...`);
      await runPath("up", cfg.upList);
      console.log(`[${cfg.expId}] DOWN...`);
  ...
  window.solPhase311_16bh_restorePerStep_tick8MicroBracket_v1 = Runner;
  console.log("✅ Installed: solPhase311_16bh_restorePerStep_tick8MicroBracket_v1");
  await Runner.run();
