// Phase 3.11 — 16BN14_repTransitionSweep_v1
// Purpose: isolate the rep1→rep2 “mode flip” and see how betweenRepTicks_hold controls it.
//
// Protocol:
// - multB fixed = 0.96625
// - segmentTicks = 401
// - dampUsed = 20
// - reps = 20 per condition
// - restore-per-rep = false (restore once at start of each condition+dir)
// - vary betweenRepTicks_hold across: 0, 81, 400 (separate subruns)
// - run both dirs: up + down (precondition differs by dir)
//
// Exports per subrun:
//   *_MASTER_summary.csv
...
(async () => {
  ...
  // -----------------------------
  const CFG = {
    expIdBase: "sol_phase311_16bn14_repTransitionSweep_v1",
    ...
    // read ticks (for per-rep curve snapshots)
    readTicks: [10, 12, 14, 20, 80, 120, 140, 154, 155, 160, 200, 216, 217, 240, 280, 320, 360, 400],
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[BN14] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[BN14] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[BN14] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[BN14] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  const runOneRep = async (
    ...
    // per-rep curve row
    const winners = CFG.readTicks.map(t => readWinner.get(t) ?? 0);
    curveLines.push(csvRow([
      CFG.expIdBase, runTag, holdTicks, dirTag, rep, multBUsed.toFixed(5),
      ...winners.map(w => (w === 114 ? 1 : 0)), // p114 indicator per tick (since single trial)
      cap114, cap136, lateUp114, lateDecay136_after114,
      winner_peakBus
    ]));

    if (!aborted) await relax(phy, pressC, holdTicks);

    return { aborted, cap114, cap136, lateUp114, lateDecay136_after114, winner_peakBus };
  };
  ...
  const runHoldCondition = async (phy, pressCUsed, holdTicks) => {
    ...
    const curveHeader = [
      ...
      "captureTick_114","captureTick_136","lateCaptureTick_114","lateDecayTick_136_after114",
      "winner_peakBus"
    ];
    ...
    console.log(`\n[BN14] START hold=${holdTicks} @ ${startTag}`);
    console.log(`[BN14] multBFixed=${CFG.multBFixed} segmentTicks=${CFG.segmentTicks} reps=${CFG.reps} dirs=${CFG.dirs.join(",")}`);
    ...
    try {
      for (const dirTag of CFG.dirs) {
        ...
        for (let rep = 1; rep <= CFG.reps; rep++) {
          ...
          if (r.aborted) {
            console.warn(`[BN14] ABORT hold=${holdTicks} dir=${dirTag} rep=${rep} (non-finite). Stopping this dir.`);
            break;
          }

          // quick console telemetry (key to spotting the rep1→rep2 flip)
          if (rep === 1 || rep === 2 || rep === 3 || rep === 5 || rep === 10 || rep === 20) {
            console.log(
              `[BN14] hold=${holdTicks} dir=${dirTag} rep=${rep} | cap114=${r.cap114} cap136=${r.cap136} lateUp=${r.lateUp114} lateDecay=${r.lateDecay136_after114} peak=${r.winner_peakBus}`
            );
          }
        }
      }
      ...
      console.log(`\n[BN14] DONE hold=${holdTicks} @ ${endTag}`);
