// Phase 3.11 — 16BN21_holdLadderReprimeBetweenPasses_v1
// Goal: test whether an ACTIVE re-prime (modeSelect + precondition) between passes resets carryover,
// since neutral washout (BN19/BN20) did not behave like a clean ERASE.
//
// Protocol (per dir):
...
// - PASS1: DESC (800->0)
// - WASHOUT: 4000 ticks (no injections)
// - REPRIME: modeSelect ONCE + precondition ONCE (no baseline restore)
// - PASS2: ASC (0->800)
// - NO restore between holds or passes
// - repsPerCondition=10
//
// Exports:
//   *_MASTER_summary.csv
...
(async () => {
  ...
  const CFG = {
    expIdBase: "sol_phase311_16bn21_holdLadderReprimeBetweenPasses_v1",
    ...
    // between passes
    washoutBetweenPassTicks: 4000,
    reprimeBetweenPasses: true,
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[BN21] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[BN21] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[BN21] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[BN21] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  const runAll = async (phy, pressCUsed) => {
    ...
    console.log("\n[BN21] START", runTag);
    console.log("[BN21] washoutBetweenPassTicks =", CFG.washoutBetweenPassTicks);
    console.log("[BN21] reprimeBetweenPasses =", CFG.reprimeBetweenPasses);
    ...
    try {
      for (const dirTag of CFG.dirs) {
        console.log(`\n[BN21] DIR=${dirTag} | baseline restore + modeSelect + precondition (one-time)`);
        ...
        for (let pi = 0; pi < passes.length; pi++) {
          ...
          console.log(`\n[BN21] PASS ${pass.passLabel} begins for DIR=${dirTag}`);
          ...
          for (let ci = 0; ci < pass.order.length; ci++) {
            ...
            console.log(`[BN21]  Condition ${conditionIndex}/${pass.order.length} hold=${holdTicks} reps=${CFG.repsPerCondition}`);
            ...
            for (let rep = 1; rep <= CFG.repsPerCondition; rep++) {
              ...
              if (r.aborted) throw new Error("[BN21] Aborted due to non-finite values.");
            }
          }

          // Between pass1 and pass2: washout + reprime
          if (pi === 0) {
            if (CFG.washoutBetweenPassTicks > 0) {
              console.log(`\n[BN21] WASHOUT between passes: ${CFG.washoutBetweenPassTicks} ticks (no injections)`);
              await relax(phy, pressCUsed, CFG.washoutBetweenPassTicks);
            }
            if (CFG.reprimeBetweenPasses) {
              console.log("\n[BN21] REPRIME between passes: modeSelect + precondition (no baseline restore)");
              await modeSelectOnce(phy, pressCUsed);
              await runPreconditionOnce(phy, pressCUsed, dirTag, baseB, ampD);
            }
          }
