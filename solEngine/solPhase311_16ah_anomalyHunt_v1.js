/* Phase 3.11.16AH — Anomaly Hunt (rare-channel / early-packet detector)
   Purpose:
     Hunt for “alternate lane” events like:
       - very early arbiter/handshake (e.g., arbiter_tick ~1, handshake_tick ~2)
       - unusually large peak bus amplitudes (peak114_abs / peak136_abs spikes)
       - unusual early top-edge routing fingerprints (e.g., 136->139 showing up)

   Default plan:
     - dampUsed ∈ {1.0, 20.0}
     - repsPerDamp = 100 (200 runs total)
     - protocol matches 16Y/16Z:
         multB=1.144, baseAmpB=4, baseAmpD=5.75, gain=22
         offset=+1 => 136 tick0, 114 tick1
         nudgeMult=0.20 (handshake tick = arbiter_tick + 1, only if arbiter owner is 136)

   Outputs:
     ...MASTER_summary.csv  (includes anomaly flags + fingerprint)
     ...MASTER_busTrace.csv (per-tick top edges + bus fluxes)

   Run:
     await solPhase311_16ah_anomalyHunt_v1.run()

   Stop:
     solPhase311_16ah_anomalyHunt_v1.stop()

   Notes:
     - Uses SOLBaseline.restore() per-run if available (best).
...
(() => {
  ...
  const T = {
    version: "sol_phase311_16ah_anomalyHunt_v1",
    _stop: false,
    ...
    cfg: {
      dt: 0.12,
      everyMs: 200,
      totalTicks: 41,      // enough for the packet + early lane diagnostics
      settleTicks: 3,
      spinWaitMs: 1.5,

      // If null: auto from invariants/UI fallback
      pressCBase: null,

      // Hunt targets
      dampUsedList: [1.0, 20.0],
      repsPerDamp: 100,
      shuffle: true,
      ...
      // “fast follow” window (classification)
      followWindowTicks: 6,

      // Anomaly flags (tunable)
      flagHandshakeTickLE: 4,      // early-lane if handshake_tick <= 4
      flagPeakAbsGE: 3.0,          // amplitude anomaly if peak114_abs or peak136_abs >= this
      fingerprintTicks: 13,        // store top-edge max1 pairs for ticks 0..12
      rareEdgeSubstrings: ["136->139", "136->10", "136->7", "104->114"],
    ...
    async _waitForPhysics(timeoutMs = 15000, pollMs = 50) {
      ...
      throw new Error("[16ah] timed out waiting for physics.");
    },

    _freezeLiveLoop() {
      const app = this._getApp();
      if (!app?.config) throw new Error("[16ah] App not ready.");
    ...
    _inject(phy, id, amt) {
      ...
      if (!n) throw new Error(`[16ah] node not found: ${id}`);
    ...
    _isFiniteEdgeSample(top2, f114_89, f114_79, f136_89, f136_79, f89_79) {
      const vals = [
    ...
    runFingerprintHasRareEdge(fp, rareList) {
      if (!fp) return 0;
      for (const s of rareList) {
        if (fp.includes(s)) return 1;
      }
      return 0;
    },
    ...
    async run(userCfg = {}) {
      ...
      // Stitch edge (tertiary link you’ve been seeing sometimes)
      ...
      const plan = [];
      for (const dampUsed of this.cfg.dampUsedList) {
        for (let r = 1; r <= this.cfg.repsPerDamp; r++) {
          plan.push({ dampUsed, rep: r });
        }
      }
      ...
      const summaryHeader = [
        ...
        "cross_peakAbs_89_79",
        "fingerprint_max1_pairs_0to12",
        "flagEarlyHandshake","flagHighPeak","flagRareEdge","flagAny",
      ...
      console.log(
        `[16ah] plan=${plan.length} | pressCBase=${pressCBase} | dampUsedList=${this.cfg.dampUsedList.join("/")}` +
        ` | repsPerDamp=${this.cfg.repsPerDamp} | ticks=${this.cfg.totalTicks} everyMs=${this.cfg.everyMs}` +
        ` | earlyHS<=${this.cfg.flagHandshakeTickLE} peak>=${this.cfg.flagPeakAbsGE}`
      );
      ...
      for (const item of plan) {
        ...
        const fpPairs = []; // top max1 pairs for first fingerprintTicks
        ...
        for (let tick = 0; tick < this.cfg.totalTicks; tick++) {
          ...
          const top2 = this._top2Edges(phy, false);

          const max1Pair = `${top2.best1.from}->${top2.best1.to}`;

          if (tick < this.cfg.fingerprintTicks) fpPairs.push(max1Pair);
          ...
          // Non-finite guard
          if (this.cfg.abortOnNonFinite && !this._isFiniteEdgeSample(top2, f114_89, f114_79, f136_89, f136_79, f89_79)) {
            runStatus = "aborted";
            abort_tick = String(tick);
            abort_reason = "non_finite_flux";
            console.warn(`[16ah] ABORT runIndex=${runIndex} damp=${dampUsed} rep=${repIndex} at tick=${tick}: non-finite flux sample.`);
            break;
          }
          ...
          if (arbiter_tick == null && busSet.has(max1Pair)) {
            ...
            // Schedule handshake on next tick, optionally only if 136 won
            const okOwner = (!this.cfg.nudgeOnlyIf136First) || (arbiter_owner === "136");
            const nextTick = tick + 1;
            const within = (nextTick < this.cfg.totalTicks) && ((nextTick - tick) <= this.cfg.maxHandshakeDelay);
            if (okOwner && within) {
        ...
        const fingerprint = fpPairs.join("|");

        const flagEarlyHandshake =
          (handshake_tick != null && Number.isFinite(handshake_tick) && handshake_tick <= this.cfg.flagHandshakeTickLE) ? 1 : 0;

        const flagHighPeak =
          ((peak114_abs >= this.cfg.flagPeakAbsGE) || (peak136_abs >= this.cfg.flagPeakAbsGE)) ? 1 : 0;

        const flagRareEdge =
          this.runFingerprintHasRareEdge(fingerprint, this.cfg.rareEdgeSubstrings);

        const flagAny = (flagEarlyHandshake || flagHighPeak || flagRareEdge) ? 1 : 0;
        ...
        // Compact progress log (and highlight anomalies)
        if (flagAny) {
          console.warn(
            `[16ah] ⚠ anomaly runIndex=${runIndex} damp=${Number(dampUsed).toFixed(2)} rep=${repIndex}` +
            ` | arb=${arbiter_owner}@${arbiter_tick} hs=${handshake_tick} | peak114=${peak114_abs.toFixed(3)} peak136=${peak136_abs.toFixed(3)}` +
            ` | flags: earlyHS=${flagEarlyHandshake} highPeak=${flagHighPeak} rareEdge=${flagRareEdge}`
          );
        } else if ((runIndex + 1) % 25 === 0) {
          console.log(`[16ah] progress ${(runIndex + 1)}/${plan.length} (latest damp=${Number(dampUsed).toFixed(2)} rep=${repIndex})`);
        }
      ...
      console.log(`✅ [16ah] DONE. Upload:\n- ${baseName}_MASTER_summary.csv\n- ${baseName}_MASTER_busTrace.csv`);
      return { baseName, runs: runIndex, stopped: this._stop, pressCUsed: pressCBase, dampUsedList: this.cfg.dampUsedList };
    }
  };

  window.solPhase311_16ah_anomalyHunt_v1 = T;
  console.log(`✅ solPhase311_16ah_anomalyHunt_v1 installed (${T.version}). Run: await solPhase311_16ah_anomalyHunt_v1.run()`);
})();
