// Phase 3.11 — 16BF (restore-per-step) tick8 bracket extension
// Goal: push above 0.990 to find tick8 x50 crossing under the SAME protocol as 16BE/16BD.
...
(async () => {
  ...
  const CFG = {
    expId: "sol_phase311_16bf_restorePerStep_tick8BracketExtend_v1",

    // 0.990 → 1.020 in 0.002 steps
    upList:   [0.990,0.992,0.994,0.996,0.998,1.000,1.002,1.004,1.006,1.008,1.010,1.012,1.014,1.016,1.018,1.020],
    downList: [1.020,1.018,1.016,1.014,1.012,1.010,1.008,1.006,1.004,1.002,1.000,0.998,0.996,0.994,0.992,0.990],
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[16BF] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[16BF] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[16BF] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[16BF] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  window.solPhase311_16bf_restorePerStep_tick8BracketExtend_v1 = Runner;
  console.log("✅ Installed: solPhase311_16bf_restorePerStep_tick8BracketExtend_v1");
  await Runner.run();
