/* Phase 3.11.16l — Ridge Scan + Onset Delay Map v1

Why:
  16k showed the whole boundary lives at ampB(114)=4 and ampD(136) around 6–7.
  16l zooms into that ridge with finer steps + higher reps + longer window to
  measure metastability + onset-time constants.

Design (~25 min):
  - FIX ampB = 4
  - Sweep ampD = [5.5, 5.75, 6.0, 6.25, 6.5, 6.75, 7.0, 7.25, 7.5, 8.0]
  - repsPerAmp = 12  (metastability stats)
  - postTicks = 60   (longer window catches late-onset only136 events)
  - everyMs = 200 => 61 ticks/run => 12.2s/run
  - runs = 10 * 12 = 120 => ~24.4 minutes
...
Run:
  solPhase311_16l.runPack()

Stop:
  solPhase311_16l.stop()
*/
(() => {
  "use strict";

  const solPhase311_16l = {
    version: "3.11.16l_ridgeScanOnsetDelay_v1",
    _stopFlag: false,
    stop() { this._stopFlag = true; console.warn("🛑 solPhase311_16l.stop(): stop flag set."); },
    ...
    cfg: {
      ...
      // ridge zoom
      ampB_fixed: 4,
      ampD_list: [5.5, 5.75, 6.0, 6.25, 6.5, 6.75, 7.0, 7.25, 7.5, 8.0],
      repsPerAmp: 12,
      ...
      filenameBase: "sol_phase311_16l_ridgeScanOnsetDelay_v1",
    },
    ...
    async _waitForPhysics({ timeoutMs = 15000, pollMs = 50 } = {}) {
      ...
      throw new Error("solPhase311_16l: timed out waiting for physics.");
    },
    ...
    _freezeLiveLoop() {
      const app = this._getApp();
      if (!app?.config) throw new Error("solPhase311_16l: App not ready.");
    ...
    _injectById(phy, id, amount) {
      ...
      if (!n) throw new Error(`solPhase311_16l: injector node not found: ${id}`);
    ...
    async runPack(userCfg = {}) {
      ...
      const app = this._getApp();
      if (!app) throw new Error("solPhase311_16l: SOLDashboard not found.");
      ...
      console.log(`🧪 Phase 3.11.16l starting (${this.version})`);
      ...
      console.log("✅ Phase 3.11.16l complete:", base2);
      return { baseName: base2, runsPlanned: plan.length, stopped: this._stopFlag };
    }
  };

  window.solPhase311_16l = solPhase311_16l;
  console.log(`✅ solPhase311_16l installed (${solPhase311_16l.version}). Run: solPhase311_16l.runPack()`);
