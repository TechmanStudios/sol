// Phase 3.11 — 16BJ (restore-per-step) HOLD-mode tick8 threshold scan + phase-set demo
// Purpose:
//   (1) Measure true multB dependence of tick8 without period-2 aliasing by forcing HOLD mode.
//       Achieved by making stride even: stride = segmentTicks(121) + betweenRepTicks(81) = 202 (even).
//   (2) Optional phase-set demo: do ONE "toggle stride" rep first, then switch to HOLD,
//       to test whether the alternate phase can be latched.
//
...
//   - phaseLatch_demo.csv   (if demo enabled)

(async () => {
  "use strict";

  const CFG = {
    expId: "sol_phase311_16bj_holdMode_tick8ThresholdScan_v1",

    // --- HOLD mode control (critical) ---
    segmentTicks: 121,
    betweenRepTicks_hold: 81,   // stride=202 even => hold phase
    betweenRepTicks_toggle: 80, // stride=201 odd => toggles phase each rep

    // --- Scan schedule ---
    // Coarse scan to locate crossing (start near 16BF/16BG shelf region and climb)
    coarse_up:   [0.985,0.987,0.989,0.991,0.993,0.995,0.997,0.999,1.001,1.003,1.005,1.007,1.009,1.011],
    coarse_down: [1.011,1.009,1.007,1.005,1.003,1.001,0.999,0.997,0.995,0.993,0.991,0.989,0.987,0.985],

    // If you later want automatic fine scan, set enableFineScan=true and define the band manually below.
    enableFineScan: false,
    fine_up:   [0.9980,0.9985,0.9990,0.9995,1.0000,1.0005,1.0010,1.0015,1.0020],
    fine_down: [1.0020,1.0015,1.0010,1.0005,1.0000,0.9995,0.9990,0.9985,0.9980],

    repsPerStep: 60,      // per multB step (HOLD mode)
    dt: 0.12,
    pressCBase: null,
    dampUsed: 20,
    markerTick: 8,
    ...
    // --- Phase-latch demo ---
    enablePhaseLatchDemo: true,
    // Two latching attempts:
    //   Demo A: start in "natural hold" (no toggle priming) => see which phase it holds (likely 136).
    //   Demo B: do one toggle-stride rep before holding => see if it flips and stays.
    demo_multB: 0.9870,
    demo_dir: "down",          // down is where we saw phase slips; good for stress-testing
    demo_reps_hold: 61,        // odd length so we can see “all same” cleanly
  };
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[16BJ] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[16BJ] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[16BJ] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[16BJ] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  // ----------------------------
  // Derived tables (same as prior runs)
  // ----------------------------
  const buildRatesByDirMultB = (rows) => {
    ...
    out.sort((x, y) => (x.dir === y.dir) ? (x.multB - x.multB) : (x.dir < y.dir ? -1 : 1));
    // OOPS guard: fix sort bug if browser keeps it
  ...
  const Runner = {
    ...
    async run(userCfg = {}) {
      ...
      const demoHeader = [
        "schema","runTag","demoCase","dir","multB","rep","betweenRepTicks",
        "winner_t8","t8_abs114_bus","t8_abs136_bus","captureTick_114","captureTick_136"
      ];

      const summaryLines = [csvRow(summaryHeader)];
      const traceLines = [csvRow(traceHeader)];
      const demoLines = [csvRow(demoHeader)];
      ...
      const runOneTrial = async (dirTag, stepIndex, rep, multBUsed, baselineModeUsed, betweenRepTicks, isDemo=false, demoCase="") => {
        ...
        if (!isDemo) {
          ...
          demoLines.push(csvRow([
            cfg.expId, runTag, demoCase, dirTag, multBUsed, rep, betweenRepTicks,
            winner_t8, t8_abs114, t8_abs136, cap114, cap136
          ]));
        }
      ...
      const runStep = async (dirTag, stepIndex, multBUsed, betweenRepTicks) => {
        ...
        for (let rep = 1; rep <= cfg.repsPerStep; rep++) {
          if (this.stopFlag) break;
          const r = await runOneTrial(dirTag, stepIndex, rep, multBUsed, baselineModeUsed, betweenRepTicks);
          if (r?.aborted) break;

          // HOLD relax stride between reps
      ...
      const runPath = async (dirTag, list, betweenRepTicks) => {
        for (let s = 0; s < list.length; s++) {
          if (this.stopFlag) break;
          await runStep(dirTag, s, list[s], betweenRepTicks);
        }
      };

      // ---- Coarse scan in HOLD mode ----
      console.log(`\n[${cfg.expId}] START @ ${startTag}`);
      console.log(`[${cfg.expId}] HOLD mode betweenRepTicks=${cfg.betweenRepTicks_hold} (stride even)`);
      console.log(`[${cfg.expId}] Coarse UP...`);
      await runPath("up", cfg.coarse_up, cfg.betweenRepTicks_hold);
      console.log(`[${cfg.expId}] Coarse DOWN...`);
      await runPath("down", cfg.coarse_down, cfg.betweenRepTicks_hold);

      // Optional fine scan (still HOLD)
      if (cfg.enableFineScan) {
        console.log(`[${cfg.expId}] Fine UP...`);
        await runPath("up_fine", cfg.fine_up, cfg.betweenRepTicks_hold);
        console.log(`[${cfg.expId}] Fine DOWN...`);
        await runPath("down_fine", cfg.fine_down, cfg.betweenRepTicks_hold);
      }
      ...
      // ---- Phase latch demo (optional) ----
      if (cfg.enablePhaseLatchDemo) {
        const doDemoCase = async (demoCase, primeToggle) => {
          if (this.stopFlag) return;
          const dirTag = cfg.demo_dir;
          const multBUsed = cfg.demo_multB;
          ...
          // Prime toggle once if requested (one rep with odd stride)
          if (primeToggle) {
            const r1 = await runOneTrial(dirTag, -1, 1, multBUsed, baselineModeUsed, cfg.betweenRepTicks_toggle, true, demoCase);
            if (r1?.aborted) return;
            for (let k = 0; k < cfg.betweenRepTicks_toggle; k++) phy.step(cfg.dt, pressCUsed, cfg.dampUsed);
          }

          // Now hold for N reps with even stride
          for (let rep = 1; rep <= cfg.demo_reps_hold; rep++) {
            const rr = await runOneTrial(dirTag, -1, rep, multBUsed, baselineModeUsed, cfg.betweenRepTicks_hold, true, demoCase);
            if (rr?.aborted) break;
            for (let k = 0; k < cfg.betweenRepTicks_hold; k++) phy.step(cfg.dt, pressCUsed, cfg.dampUsed);
          }
        };

        console.log(`[${cfg.expId}] PhaseLatch demo A (no priming)...`);
        await doDemoCase("A_holdOnly", false);

        console.log(`[${cfg.expId}] PhaseLatch demo B (1 toggle rep then hold)...`);
        await doDemoCase("B_toggleThenHold", true);
      }
      ...
      if (cfg.enablePhaseLatchDemo) downloadText(`${baseName}_phaseLatch_demo.csv`, demoLines.join(""));
      ...
      if (cfg.enablePhaseLatchDemo) console.log(`- ${baseName}_phaseLatch_demo.csv`);

      return { expId: cfg.expId, baseName, pressCUsed, dampUsed: cfg.dampUsed, stopped: this.stopFlag };
    }
  };

  window.solPhase311_16bj_holdMode_tick8ThresholdScan_v1 = Runner;
  console.log("✅ Installed: solPhase311_16bj_holdMode_tick8ThresholdScan_v1");
  await Runner.run();
