// Phase 3.11 — 16BN25_dampPortabilityMap0to22_v1p1
// Goal: map hold-ladder behavior across dampUsed = 0..22 (inclusive) with higher-fidelity sampled busTrace.
//
// Design (per damp, per dir):
// - baseline restore
// - modeSelect
// - precondition
// - PASS1: DESC (800->0)
// - PASS2: ASC (0->800)
// - no baseline restore between passes or holds
// - repsPerCondition default = 5 (summary/curve remain full-fidelity)
//
// Exports:
//   *_MASTER_summary.csv           (FULL reps)
//   *_repTransition_curve.csv      (FULL reps)
...
(async () => {
  ...
  const CFG = {
    expIdBase: "sol_phase311_16bn25_dampPortabilityMap0to22_v1p1",

    dt: 0.12,
    pressCBase: null,

    // Sweep damp = 0..22
    dampList: Array.from({ length: 23 }, (_, i) => i),

    // Hold ladder
    holdsAsc: [0, 40, 81, 160, 400, 800],

    // Reps per condition (FULL fidelity for summary + curve)
    repsPerCondition: 5,

    // Directions
    dirs: ["up", "down"],

    // Optional settle (kept 0 for comparability)
    betweenConditionSettleTicks: 0,

    // ---- Higher-fidelity busTrace sampling knobs ----
    // Record every N ticks (lower = more fidelity, larger file)
    traceStride: 4,              // was 5
    // Record trace only for first N reps per condition
    traceRepsPerCondition: 2,    // keep 2 to stay ~40–45MB
    ...
    // Robustness: do NOT hard-stop the sweep on a single non-finite rep.
    // Instead mark aborted=1 and continue.
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[BN25] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[BN25] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[BN25] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[BN25] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  const runAll = async (phy, pressCUsed) => {
    ...
    const asc = CFG.holdsAsc.slice();
    const desc = CFG.holdsAsc.slice().reverse();

    const passes = [
      { passId: 1, passLabel: "DESC", order: desc, orderLabel: "DESC_" + desc.join("_") },
      { passId: 2, passLabel: "ASC",  order: asc,  orderLabel: "ASC_"  + asc.join("_")  }
    ];

    // Estimate busTrace row count for sanity:
    // conditions = dampCount * dirs * passes * holds
    const conditions = CFG.dampList.length * CFG.dirs.length * passes.length * CFG.holdsAsc.length;
    const rowsPerTracedRep = Math.floor(CFG.segmentTicks / CFG.traceStride) + 1;
    const estTraceRows = conditions * CFG.traceRepsPerCondition * rowsPerTracedRep;

    console.log("\n[BN25] START", runTag);
    console.log("[BN25] damp sweep:", CFG.dampList.join(","));
    console.log("[BN25] trace sampling: stride=", CFG.traceStride, "repsPerCond=", CFG.traceRepsPerCondition);
    console.log("[BN25] estTraceRows ~", estTraceRows);
    ...
    try {
      for (const dampUsed of CFG.dampList) {
        console.log(`\n[BN25] === dampUsed=${dampUsed} ===`);

        for (const dirTag of CFG.dirs) {
          console.log(`[BN25] DIR=${dirTag} | baseline restore + modeSelect + precondition`);
          ...
          for (const pass of passes) {
            console.log(`[BN25] PASS ${pass.passLabel} | holds=${pass.order.join(",")} | reps=${CFG.repsPerCondition}`);
            ...
            for (let ci = 0; ci < pass.order.length; ci++) {
              ...
              if (CFG.betweenConditionSettleTicks > 0) {
                await relax(phy, pressCUsed, dampUsed, CFG.betweenConditionSettleTicks);
              }
