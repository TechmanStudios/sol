// Phase 3.11 — 16BN3 (Latch basin inversion extension)
// Goal: Extend the odd-toggle latch scan downward to capture the 0.5 crossing and inversion
// (where tick8 becomes >50% 136 even after the odd-toggle prime).
//
// Window: multB 0.9720 → 0.9650 in -0.0005 steps (15 points)
...
//   - latch_break_estimate.csv (here it's more like "threshold bracket" + notes)
//
...
(async () => {
  ...
  const CFG = {
    expId: "sol_phase311_16bn3_latchBasinInversionScan_v1",
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[16BN3] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[16BN3] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[16BN3] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[16BN3] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  // Find x50 bracket for p114_t8 crossing 0.5 in this scan direction
  const estimateX50 = (rows) => {
    // rows must be in descending multB order
    for (let i = 0; i < rows.length - 1; i++) {
      const a = rows[i];
      const b = rows[i + 1];
      // looking for crossing around 0.5
      const pa = a.p114_t8 - 0.5;
      const pb = b.p114_t8 - 0.5;
      if (pa === 0) return { x50: a.multB, bracketLo: a.multB, bracketHi: a.multB, note: "exact" };
      if (pa > 0 && pb < 0) {
        // linear interpolation
        const t = pa / (pa - pb);
        const x = a.multB + t * (b.multB - a.multB);
        return {
          x50: x,
          bracketLo: Math.min(a.multB, b.multB),
          bracketHi: Math.max(a.multB, b.multB),
          note: "interp"
        };
      }
    }
    return { x50: "", bracketLo: "", bracketHi: "", note: "no_crossing_in_window" };
  };
    ...
    async run(userCfg = {}) {
      ...
      for (const dirTag of dirs) {
        ...
        for (let stepIndex = 0; stepIndex < multBList.length; stepIndex++) {
          ...
          if (abortedAny) {
            console.warn(`[${cfg.expId}] ABORTED at dir=${dirTag} multB=${multBUsed}`);
            break;
          }
          ...
          console.log(
            `[${cfg.expId}] ${dirTag} step ${stepIndex}/${multBList.length-1} multB=${multBUsed.toFixed(4)} p114_t8=${a.p114_t8.toFixed(4)}`
          );
        }

        // Estimate x50 within this window (if it crosses)
        const x = estimateX50(dirRows);
        estLines.push(csvRow([
          cfg.expId, runTag, dirTag,
          x.x50 !== "" ? x.x50 : "",
          x.bracketLo !== "" ? x.bracketLo : "",
          x.bracketHi !== "" ? x.bracketHi : "",
          x.note
        ]));
      }
      ...
      downloadText(`${baseName}_latch_break_estimate.csv`, estLines.join(""));
  ...
  window.solPhase311_16bn3_latchBasinInversionScan_v1 = Runner;
  console.log("✅ Installed: solPhase311_16bn3_latchBasinInversionScan_v1");
  await Runner.run();
