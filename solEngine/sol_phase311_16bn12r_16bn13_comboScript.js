// Phase 3.11 — Batch: BN12R + BN13
// BN12R: segmentTicks sweep WITH restore-per-rep (M0), to test whether 241 “bubble→decay” is cross-rep memory.
// BN13 : decay-time map under “bubble protocol” (M0, hold400), extend segmentTicks to 401 and measure the down-cross.
//
// Exports per subrun:
//  - *_MASTER_summary.csv
...
(async () => {
  ...
  // -----------------------------
  // Subrun configs (edit here)
  // -----------------------------
  const SUBRUNS = [
    // BN12R — segmentTicks sweep, restore per rep
    {
      expId: "sol_phase311_16bn12r_segmentTicksSweep_restorePerRep_M0_v1",
      ...
      // For seg sweep, we focus on lateCaptureTick_114 & decay tick (see summary columns)
      readTicks: [10, 11, 12, 13, 14, 20, 40, 80, 120, 140, 160, 200, 220, 240],
    },

    // BN13 — decay-time map under hold400 protocol (bubble then decay), long segment
    {
      expId: "sol_phase311_16bn13_decayTimeMap_hold400_M0_v1",
      curveName: "decayTime_map_curve",
      modeLabel: "M0_noPrime",
      primeCount: 0,
      restorePerRep: false,
      ...
      dirs: ["up", "down"],
      repsPerStep: 80,
      multBList: [0.96575, 0.96625, 0.96675], // 3-point map around seam
      segmentTicks: 401,
      // Reads that let you see both the bubble and the decay without scanning every tick in the curve
      readTicks: [10, 12, 14, 20, 120, 140, 160, 180, 200, 216, 240, 280, 320, 360, 400],
    },
  ];

  // -----------------------------
  // Shared defaults / invariants
  // -----------------------------
  const DEFAULTS = {
    dt: 0.12,
    pressCBase: null,
    ...
    // “late window” detection start (matches prior work)
    lateStartTick: 120,
    ...
    // toggle and hold timing (toggle used only if primeCount>0; these runs are M0)
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[BN12R+BN13] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[BN12R+BN13] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[BN12R+BN13] node not found: ${id}`);
  ...
  // -----------------------------
  // Baseline restore (fallback snapshot)
  // -----------------------------
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[BN12R+BN13] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  // -----------------------------
  // Mode select + precondition (same as prior)
  // -----------------------------
  ...
  // -----------------------------
  // One trial runner (records both late up-cross and late decay)
  // -----------------------------
  const runOneTrial = async (
    ...
    // Late “up-cross” (114 capture) and “decay” (136 capture after 114 recapture)
    const lateUp114 = captureTick(busDomSeq, "114", cfg.captureStreak, cfg.lateStartTick);

    let lateDecay136_after114 = "";
    if (lateUp114 !== "") {
      const start = Math.max(cfg.lateStartTick, (Number(lateUp114) + 10) | 0);
      lateDecay136_after114 = captureTick(busDomSeq, "136", cfg.captureStreak, start);
    }

    // summary row includes: lateUp114 + lateDecay136_after114
    summaryLines.push(csvRow([
      ...
      cfg.betweenRepTicks_hold,
      aborted ? 1 : 0,
      lateUp114, cfg.lateStartTick,
      lateDecay136_after114
    ]));
    ...
    return { aborted, winner_peakBus, cap114, cap136, lateUp114, lateDecay136_after114, winnersByTick };
  };
  ...
  const runSubrun = async (phy, baseCfg, sub) => {
    ...
    // Curve formats:
    // - segmentTicks_sweep_curve: per segTicks+dir, report distributions of lateUp114 and lateDecay
    // - decayTime_map_curve: per multB+dir, report distributions of lateUp114 and lateDecay + key read tick probabilities
    const curveHeader_seg = [
      "schema","runTag","dir","segmentTicks","multB","modeLabel","primeCount","n",
      "q25_lateUp114","q50_lateUp114","q75_lateUp114","mean_lateUp114","nanRate_lateUp114",
      "q25_lateDecay136","q50_lateDecay136","q75_lateDecay136","mean_lateDecay136","nanRate_lateDecay136",
      "q25_cap114","q50_cap114","q75_cap114","mean_cap114","nanRate_cap114"
    ];

    const curveHeader_decay = [
      "schema","runTag","dir","segmentTicks","multB","modeLabel","primeCount","n",
    ...
    const curveLines = [csvRow(cfg.curveName === "segmentTicks_sweep_curve" ? curveHeader_seg : curveHeader_decay)];

    const segmentTicksList = cfg.segmentTicksList ? cfg.segmentTicksList.slice() : [cfg.segmentTicks];

    console.log(`\n[${cfg.expId}] START @ ${startTag}`);
    console.log(`[${cfg.expId}] restorePerRep=${!!cfg.restorePerRep} betweenRepHold=${cfg.betweenRepTicks_hold}`);
    console.log(`[${cfg.expId}] dirs=${cfg.dirs.join(",")} multBList=[${cfg.multBList.join(", ")}] segList=[${segmentTicksList.join(", ")}] reps=${cfg.repsPerStep}`);
      ...
      for (const segTicks of segmentTicksList) {
        ...
        for (const dirTag of cfg.dirs) {
          for (const multBUsedRaw of cfg.multBList) {
            ...
            const cap114_list = [];
            const lateUp_list = [];
            const lateDecay_list = [];

            let nCap114_nan = 0;
            let nLateUp_nan = 0;
            let nLateDecay_nan = 0;

            let abortedAny = false;
            let baselineModeUsed = "N/A";

            // STEP INIT (only if not restorePerRep)
            ...
            for (let rep = 1; rep <= cfg.repsPerStep; rep++) {
              ...
              const lu = (r.lateUp114 === "" ? NaN : Number(r.lateUp114));
              const ld = (r.lateDecay136_after114 === "" ? NaN : Number(r.lateDecay136_after114));

              cap114_list.push(c114);
              lateUp_list.push(lu);
              lateDecay_list.push(ld);

              if (!Number.isFinite(c114)) nCap114_nan++;
              if (!Number.isFinite(lu)) nLateUp_nan++;
              if (!Number.isFinite(ld)) nLateDecay_nan++;
            ...
            const q25_cap114 = quantile(cap114_list, 0.25);
            const q50_cap114 = quantile(cap114_list, 0.50);
            const q75_cap114 = quantile(cap114_list, 0.75);
            const mu_cap114 = meanNum(cap114_list);
            const nanRate_cap114 = nCap114_nan / n;

            const q25_lu = quantile(lateUp_list, 0.25);
            const q50_lu = quantile(lateUp_list, 0.50);
            const q75_lu = quantile(lateUp_list, 0.75);
            const mu_lu = meanNum(lateUp_list);
            const nanRate_lu = nLateUp_nan / n;

            const q25_ld = quantile(lateDecay_list, 0.25);
            const q50_ld = quantile(lateDecay_list, 0.50);
            const q75_ld = quantile(lateDecay_list, 0.75);
            const mu_ld = meanNum(lateDecay_list);
            const nanRate_ld = nLateDecay_nan / n;
            ...
            if (cfg.curveName === "segmentTicks_sweep_curve") {
              curveLines.push(csvRow([
                ...
                q25_lu, q50_lu, q75_lu, mu_lu, nanRate_lu,
                q25_ld, q50_ld, q75_ld, mu_ld, nanRate_ld,
                q25_cap114, q50_cap114, q75_cap114, mu_cap114, nanRate_cap114
              ]));
            } else {
              curveLines.push(csvRow([
                cfg.expId, runTag, dirTag, cfg.segmentTicks, multBUsed.toFixed(5), cfg.modeLabel, cfg.primeCount, n,
            ...
            console.log(
              `[${cfg.expId}] ${cellId} | lateUp(q50)=${q50_lu} lateDecay(q50)=${q50_ld} nanDecay=${nanRate_ld.toFixed(2)}`
              + (abortedAny ? " [ABORT]" : "")
            );
          }
        }
