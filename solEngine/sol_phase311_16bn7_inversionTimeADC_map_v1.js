// Phase 3.11 — 16BN7 (Inversion-time ADC map)
// Goal: Treat "time-to-114-capture" as an ADC-like transfer function.
// We sweep multB across the basin boundary band and measure, per condition:
//   - median captureTick_114 (streak=5)
//   - p114 at fixed read ticks {9,10,11,12,13}
// Conditions: M0 (0 primes) vs M1 (1 prime) only.
// Directions: BOTH (up + down)
// Reps/step: 80
// Sweep: multB from 0.9720 down to 0.9640 by -0.0005
//
// Exports:
//   - MASTER_summary.csv
//   - MASTER_busTrace.csv
//   - inversionTime_ADC_curve.csv  (per multB × dir × mode)
//
...
(async () => {
  ...
  const CFG = {
    expId: "sol_phase311_16bn7_inversionTimeADC_map_v1",
    ...
    // fixed read ticks (for p114(t))
    readTicks: [9, 10, 11, 12, 13],
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[16BN7] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[16BN7] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[16BN7] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[16BN7] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  const modeLabelOf = (primeCount) => (primeCount === 0) ? "M0_noPrime" : "M1_togglePrime";

  // ---------- One recorded trial (also returns per-readTick winners + captureTick_114) ----------
  const runOneTrial = async (
    ...
    // store abs at selected read ticks
    ...
    // summary row includes read tick8 style fields replaced by blanks (we're focusing on inversion-time + p114(t))
    ...
    return { aborted, winner_peakBus, cap114, winnersByTick };
  };
  ...
  const buildSweep = (start, end, step) => {
    ...
    // inclusive-ish: stop when we pass end in direction of step
    let x = start;
    for (let k = 0; k < 5000; k++) {
      xs.push(Math.round(x * 10000) / 10000);
      x = x + step;
  ...
  // ADC curve output
  // p114 at read ticks, plus median captureTick_114 (streak=5) and median captureTick_136
  const adcHeader = [
    "schema","runTag","dir","multB","modeLabel","primeCount",
    "n",
    "p114_t9","p114_t10","p114_t11","p114_t12","p114_t13",
  ...
  const adcLines = [csvRow(adcHeader)];
  ...
  console.log(`[${CFG.expId}] sweep: ${sweep[0].toFixed(4)} → ${sweep[sweep.length-1].toFixed(4)} (step ${CFG.multB_step}) | repsPerStep=${CFG.repsPerStep}`);
  console.log(`[${CFG.expId}] modes: ${CFG.primeCounts.join(",")} | dirs: ${dirs.join(",")} | readTicks=${CFG.readTicks.join(",")}`);

  try {
    for (const dirTag of dirs) {
      for (const multBUsed of sweep) {
        for (const primeCount of CFG.primeCounts) {
          ...
          for (let rep = 1; rep <= CFG.repsPerStep; rep++) {
            ...
            // cap136 is in summary but not returned; we can still approximate by leaving blank or re-deriving later.
            // Here we leave cap136 median blank (computed from MASTER_summary post-hoc).
          ...
          const med_cap136 = ""; // compute from MASTER_summary later for highest fidelity

          adcLines.push(csvRow([
            CFG.expId, runTag, dirTag, multBUsed.toFixed(4), modeLabel, primeCount,
            nPeak,
            p114_t(9), p114_t(10), p114_t(11), p114_t(12), p114_t(13),
            med_cap114, med_cap136,
            p114_peak
          ]));

          console.log(
            `[${CFG.expId}] ${cellId} | p114@10=${p114_t(10).toFixed(3)} p114@12=${p114_t(12).toFixed(3)} | medCap114=${med_cap114}`
            + (abortedAny ? " [ABORT]" : "")
          );
        }
      }
    ...
    downloadText(`${baseName}_inversionTime_ADC_curve.csv`, adcLines.join(""));

    console.log(`\n[${CFG.expId}] DONE @ ${endTag}`);
    console.log(`- ${baseName}_MASTER_summary.csv`);
    console.log(`- ${baseName}_MASTER_busTrace.csv`);
    console.log(`- ${baseName}_inversionTime_ADC_curve.csv`);
  }
