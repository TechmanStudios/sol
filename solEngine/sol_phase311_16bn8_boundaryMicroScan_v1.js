// Phase 3.11 — 16BN8 (Boundary micro-scan + expanded read ticks)
// Goal: Micro-scan multB around the BN7 boundary (where M0 crosses p114@tick12 ~ 0.5)
// and measure p114 at ticks {10,11,12,13,14}, plus captureTick_114 stats.
//
// Sweep: multB = 0.9655 → 0.9680 step +0.00025  (10 points + endpoints)
...
(async () => {
  ...
  const CFG = {
    expId: "sol_phase311_16bn8_boundaryMicroScan_v1",
    ...
    // fixed read ticks (expanded)
    readTicks: [10, 11, 12, 13, 14],
  ...
  const waitForPhysics = async (timeoutMs = 15000, pollMs = 50) => {
    ...
    throw new Error("[16BN8] timed out waiting for physics.");
  };

  const freezeLiveLoop = () => {
    const app = getApp();
    if (!app?.config) throw new Error("[16BN8] App not ready.");
  ...
  const injectRho = (phy, id, amt) => {
    ...
    if (!n) throw new Error(`[16BN8] node not found: ${id}`);
  ...
  const ensureBaselineIfAvailable = async () => {
    ...
    catch (e) { console.warn("[16BN8] SOLBaseline.ensure failed (continuing):", e); }
  };
  ...
  // ---------- One recorded trial (returns per-readTick winners + capture ticks) ----------
  ...
  const buildSweep = (start, end, step) => {
    ...
    for (let k = 0; k < 5000; k++) {
      xs.push(Math.round(x * 100000) / 100000); // keep finer internal precision
      x = x + step;
      if ((dir > 0 && x > end + 1e-12) || (dir < 0 && x < end - 1e-12)) break;
    }
    // round to 5 decimals in output, but still safe
  ...
  const adcHeader = [
    ...
    "n",
    "p114_t10","p114_t11","p114_t12","p114_t13","p114_t14",
    "median_captureTick_114","nanRate_captureTick_114",
    "median_captureTick_136","nanRate_captureTick_136",
    "p114_peak"
  ];
  ...
  console.log(`[${CFG.expId}] sweep: ${sweep[0].toFixed(5)} → ${sweep[sweep.length-1].toFixed(5)} (step ${CFG.multB_step}) | repsPerStep=${CFG.repsPerStep}`);
    ...
    for (const dirTag of dirs) {
      for (const multBRaw of sweep) {
        ...
        for (const primeCount of CFG.primeCounts) {
          ...
          const cellId = `${dirTag}_B${multBUsed.toFixed(5)}_${modeLabel}`;
          ...
          adcLines.push(csvRow([
            CFG.expId, runTag, dirTag, multBUsed.toFixed(5), modeLabel, primeCount,
            nPeak,
            p114_t(10), p114_t(11), p114_t(12), p114_t(13), p114_t(14),
            med_cap114, nanRate114,
            med_cap136, nanRate136,
            p114_peak
          ]));

          console.log(
            `[${CFG.expId}] ${cellId} | p114@12=${p114_t(12).toFixed(3)} p114@13=${p114_t(13).toFixed(3)} | medCap114=${med_cap114} nan114=${nanRate114.toFixed(2)}`
            + (abortedAny ? " [ABORT]" : "")
          );
        }
      }
