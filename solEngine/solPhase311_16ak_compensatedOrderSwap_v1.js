/* Phase 3.11.16AK — Compensated Order Swap (114-first with amplitude sweep)
   Goal:
     Test whether 114 can seize arbitration when it:
       - injects FIRST (114@t0, 136@t1), and
       - is driven harder (sweep multB)

   Conditions:
     - dampUsed ∈ {12, 20}
     - multBUsed ∈ {1.144, 1.44, 1.7, 2.0}
     - repsPerCond = 40 (default)
     - handshake nudge: inject 114 by nudgeMult*ampB0 at handshake_tick (= arbiter_tick+1)
       - nudgeOnlyIf136First = false (important: don’t gate handshake by winner)

   Outputs (2 files, downloaded after completion):
     - ..._MASTER_summary.csv
     - ..._MASTER_busTrace.csv

   Run:
     await solPhase311_16ak_compensatedOrderSwap_v1.run()

   Stop:
     solPhase311_16ak_compensatedOrderSwap_v1.stop()
...
(() => {
  ...
  const T = {
    version: "sol_phase311_16ak_compensatedOrderSwap_v1",
    _stop: false,
    ...
    cfg: {
      ...
      // Mode-select priming (consistent with 16Y/16Z/16AH/16AI/16AJ)
      wantId: 82,
      injectorIds: [90, 82],
      dreamBlocks: 15,
      dreamBlockSteps: 2,
      ...
      // 16AK sweep vars
      dampUsedList: [12, 20],
      multBUsedList: [1.144, 1.44, 1.7, 2.0],
      repsPerCond: 40,
      shuffle: true,

      // Order swap: 114 first, then 136
      injectTick114: 0,
      injectTick136: 1,
      ...
      // Marker moment (useful to see whether 139-gate appears / disappears)
      markerTickToCheck: 8,
    ...
    async _waitForPhysics(timeoutMs = 15000, pollMs = 50) {
      ...
      throw new Error("[16ak] timed out waiting for physics.");
    },

    _freezeLiveLoop() {
      const app = this._getApp();
      if (!app?.config) throw new Error("[16ak] App not ready.");
    ...
    _inject(phy, id, amt) {
      ...
      if (!n) throw new Error(`[16ak] node not found: ${id}`);
    ...
    async run(userCfg = {}) {
      ...
      // Plan: damp × multB × reps
      const plan = [];
      for (const dampUsed of this.cfg.dampUsedList) {
        for (const multBUsed of this.cfg.multBUsedList) {
          for (let r = 1; r <= this.cfg.repsPerCond; r++) {
            plan.push({ dampUsed, multBUsed, rep: r });
          }
        }
      }
      if (this.cfg.shuffle) this._shuffle(plan);

      // Constant ampD; ampB0 depends on multBUsed
      const baseB = this.cfg.baseAmpB * this.cfg.gain; // 4*22 = 88
      const baseD = this.cfg.baseAmpD * this.cfg.gain; // 5.75*22 = 126.5
      ...
      const summaryHeader = [
        ...
        "injectTick114","injectTick136","nudgeMult","nudgeOnlyIf136First",
      ...
      console.log(
        `[16ak] plan=${plan.length} | damps=[${this.cfg.dampUsedList.join(",")}]` +
        ` | multB=[${this.cfg.multBUsedList.join(",")}] reps=${this.cfg.repsPerCond}` +
        ` | inject(114@${this.cfg.injectTick114},136@${this.cfg.injectTick136})` +
        ` | nudgeMult=${this.cfg.nudgeMult} nudgeOnlyIf136First=${this.cfg.nudgeOnlyIf136First}`
      );
      ...
      for (const item of plan) {
        ...
        for (let tick = 0; tick < this.cfg.totalTicks; tick++) {
          ...
          // Injections (pre-step)
          if (tick === this.cfg.injectTick114) this._inject(phy, 114, ampB0);
          if (tick === this.cfg.injectTick136) this._inject(phy, 136, ampD);
          ...
          if (tick === this.cfg.markerTickToCheck) {
            max1_at_markerTick = max1Pair;
            laneClass = this._laneClassFromMarker(max1Pair);
          }
          if (!first_marker_tick && this.cfg.markerEdges.includes(max1Pair)) first_marker_tick = String(tick);
          ...
          if (this.cfg.abortOnNonFinite) {
            ...
            if (!ok) {
              ...
              console.warn(`[16ak] ABORT runIndex=${runIndex} damp=${dampUsed} multB=${multBUsed} rep=${repIndex} tick=${tick}: non-finite sample`);
              break;
            }
          }
        ...
        summaryLines.push(this._csvRow([
          ...
          this.cfg.gain, baseB, baseD, multBUsed, this.cfg.multD,
          ampB0, ampD, ratioBD, ampDiff,
          this.cfg.injectTick114, this.cfg.injectTick136, this.cfg.nudgeMult, (this.cfg.nudgeOnlyIf136First ? 1 : 0),
        ...
        // Progress log (highlight any 114-first or 114-arbiter events immediately)
        if (arbiterOwner114 || precedenceStarts114) {
          console.warn(
            `[16ak] 🧲 114-event runIndex=${runIndex} damp=${Number(dampUsed).toFixed(2)} multB=${Number(multBUsed).toFixed(3)} rep=${repIndex}` +
            ` | arb=${arbiter_owner}@${arbiter_tick} hs=${handshake_tick} prec=${precedence} class=${packetClass}` +
            ` | ampB0=${ampB0.toFixed(3)} ampD=${ampD.toFixed(3)}`
          );
        } else if ((runIndex + 1) % 25 === 0) {
          console.log(`[16ak] progress ${(runIndex + 1)}/${plan.length}`);
        }
      ...
      return { baseName, pressCUsed: pressCBase, runs: runIndex, stopped: this._stop };
    }
  };

  window.solPhase311_16ak_compensatedOrderSwap_v1 = T;
  console.log(`✅ solPhase311_16ak_compensatedOrderSwap_v1 installed (${T.version}). Run: await solPhase311_16ak_compensatedOrderSwap_v1.run()`);
})();
