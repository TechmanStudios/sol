/* Phase 3.11.16y — Adaptive Handshake Damp Sweep (Multistage Probe)
   Idea:
     Stage 1: Arbitration moment = first tick where bus max1 appears (114/136 → 89/79).
     Stage 2: Handshake = nudge 114 on the next tick (self-timed).
     Stage 3: Observe whether 114 rail appears quickly; track 89->79 stitch growth.

   Fixed protocol (from 16w winner):
     - multB=1.144, baseAmpB=4, baseAmpD=5.75, gain=22
     - offset=+1 => 136 at tick0, 114 at tick1
     - nudgeMult=0.20 (applied adaptively: arbiterTick+1)
     - Expected at damp 4–5: arbiterTick~13, 114 shows ~15 (Δ=2)

   Sweep:
     - dampUsedList: [4,5,6,8,10,12,15,20]
     - pressCUsed: base from invariants/UI (fixed at base to isolate damping)

   Run:
     await solPhase311_16y_adaptiveHandshakeDampSweep_v1.run()

   Stop:
     solPhase311_16y_adaptiveHandshakeDampSweep_v1.stop()
...
(() => {
  ...
  const T = {
    version: "sol_phase311_16y_adaptiveHandshakeDampSweep_v1",
    _stop: false,
    ...
    cfg: {
      ...
      // Adaptive handshake
      nudgeMult: 0.20,
      nudgeOnlyIf136First: true,  // preserve “meaning” of the packet
      maxHandshakeDelay: 10,      // if arbiter happens late, still allow handshake
    ...
    async _waitForPhysics(timeoutMs = 15000, pollMs = 50) {
      ...
      throw new Error("[16y] timed out waiting for physics.");
    },

    _freezeLiveLoop() {
      const app = this._getApp();
      if (!app?.config) throw new Error("[16y] App not ready.");
    ...
    _inject(phy, id, amt) {
      ...
      if (!n) throw new Error(`[16y] node not found: ${id}`);
    ...
    async run(userCfg = {}) {
      ...
      console.log(
        `[16y] plan=${plan.length} | pressCBase=${pressCBase} | dampUsedList=${this.cfg.dampUsedList.join("/")}` +
      ...
      for (const item of plan) {
        ...
        for (let tick = 0; tick < this.cfg.totalTicks; tick++) {
          ...
          const max2Pair = `${top2.best2.from}->${top2.best2.to}`;
        ...
        if (runIndex % 24 === 0) {
          console.log(
            `[16y] progress ${runIndex}/${plan.length} | damp=${Number(dampUsed).toFixed(2)} | arb=${arbiter_owner}@${arbiter_tick} hs=${handshake_tick}` +
            ` | prec=${precedence} class=${packetClass} Δ=${deltaTicks} | stitchPeak=${cross_peakAbs_89_79.toExponential(3)}`
          );
        }
      }
      ...
      console.log(`✅ [16y] DONE. Upload:\n- ${baseName}_MASTER_summary.csv\n- ${baseName}_MASTER_busTrace.csv`);
      return { baseName, runs: runIndex, stopped: this._stop, pressCUsed: pressCBase, dampUsedList: this.cfg.dampUsedList };
    }
  };

  window.solPhase311_16y_adaptiveHandshakeDampSweep_v1 = T;
  console.log(`✅ solPhase311_16y_adaptiveHandshakeDampSweep_v1 installed (${T.version}). Run: await solPhase311_16y_adaptiveHandshakeDampSweep_v1.run()`);
})();
